...
function rowToAgent(r) {
  ...
  return {
    ...
    deliveryCompanyId: r.delivery_company_id,
    payoutPercent: r.payout_percent !== null && r.payout_percent !== undefined ? Number(r.payout_percent) : null,
...
const db = {
  ...
  // payoutPercent: undefined = no percentage set on creation (the normal
  // case — see the agent:create comment in server.js for why this is
  // only ever sent by a delivery_company account about its own new
  // agent). Written through as-is otherwise (server.js has already
  // range-validated it).
  async createAgent({ id, name, phone, deliveryCompanyId, payoutPercent }) {
    const { rows } = await pool.query(
      `INSERT INTO agents (id, name, phone, delivery_company_id, payout_percent) VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [id, name, phone, deliveryCompanyId || null, payoutPercent !== undefined ? payoutPercent : null]
    );
    return rowToAgent(rows[0]);
  },
  ...
  //
  // payoutPercent: same "undefined = leave unchanged" convention. Only
  // ever sent (non-undefined) by a delivery_company account editing its
  // own agent — see the agent:update comment in server.js. null
  // explicitly clears a previously-set percentage back to "not set".
  async updateAgent(id, { name, phone, deliveryCompanyId, payoutPercent }) {
    const setClauses = ['name = $1', 'phone = $2'];
    const values = [name, phone];
    if (deliveryCompanyId !== undefined) {
      values.push(deliveryCompanyId);
      setClauses.push(`delivery_company_id = $${values.length}`);
    }
    if (payoutPercent !== undefined) {
      values.push(payoutPercent);
      setClauses.push(`payout_percent = $${values.length}`);
    }
    values.push(id);
    const { rows } = await pool.query(
