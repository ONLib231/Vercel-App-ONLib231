...
-- Rider payout percentage, for delivery companies that pay their agents
-- a commission on the delivery fee they bring in rather than a flat
-- wage. Set only by the delivery company that owns the agent (Super
-- Admin has no control over this field, by product decision) and
-- applied against orders.amount only — orders.tip_amount is explicitly
-- "not commissionable" (see below) and is deliberately excluded here
-- too. NULL = no percentage set yet; the Monthly Report PDF still lists
-- such a rider's orders, it just can't compute a payout amount for
-- them, and doesn't guess one — same "not reliably attributable, so
-- don't guess" precedent as orders.agent_id below.
ALTER TABLE agents ADD COLUMN IF NOT EXISTS payout_percent NUMERIC(5,2) CHECK (payout_percent IS NULL OR (payout_percent >= 0 AND payout_percent <= 100));
