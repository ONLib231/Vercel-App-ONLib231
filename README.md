...
**The fix: relocate the live DOM node by role, not clone it.** `#available-orders-section` and the Recent Deliveries `<section>` were wrapped in one container, `#admin-orders-block`, left in its original inline position in the static HTML (Manage Agent's default, unchanged). A new empty marker div, `#admin-orders-inline-anchor`, sits right after it purely to remember where it goes back to. The Revenue Overview / Order Status charts (previously nested inside the same two-column layout, right under Available Orders) were pulled out into their own full-width `.admin-charts-row`, since those stay on the Dashboard for both roles regardless of where `#admin-orders-block` currently lives. At the same point login already computes `isSuperAdmin` and toggles every other role-gated nav item, a new block does one of two things, idempotently (safe to re-run on every login): for Super Admin, `orders-modal-body.appendChild(ordersBlock)` — moving the actual live node (not `innerHTML`-cloning it) into the new `#orders-modal`, so every id, event listener, and cached DOM reference (`orders-grouped-delivery`, `recent-deliveries-container`, the order search/filter inputs, Select All/Delete Selected) keeps working exactly as before, just inside a modal instead of inline; for Manage Agent, `ordersInlineAnchor.parentElement.insertBefore(ordersBlock, ordersInlineAnchor)` — restoring it to its original inline spot if it had been moved out (e.g. a Super Admin account was demoted, or the same browser session re-enters as a different role).
