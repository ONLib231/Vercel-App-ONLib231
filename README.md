...
## Self-service Zone Search Picker — Vendors, Delivery Companies, and Customers search their own Region/Zone instead of typing an address blind

**Requested:** "I need Venders, Delivery companies, and Customers to be something like a search bar where they import their addresses by searching their Regions/Zones." Scoped via a round of clarifying questions before any code was touched, since the three roles started from very different places (see below): keep each role's existing free-text address field and add the Zone search bar alongside it, not replace it; for delivery companies — who had no address concept at all — the zone represents their company's home base, same as a vendor's store address; the picker should be available both at registration and later from Settings; and a zone stays purely descriptive for now — it does not feed into delivery fee calculation, which stays 100% vendor-zone-driven exactly as before.

**Starting point per role, investigated before writing anything.** Vendors already had a free-text `storeAddress` and could be assigned one `delivery_zone_id`, but only by the Super Admin, from the Vendors table — no self-service. Delivery companies had *no* address field anywhere (schema, registration, or Settings) and no zone concept at all. Customers had a real `saved_addresses` book (label + free-text address, one default) used to autofill checkout's dropoff field, also with no zone tie-in — delivery fee has always been calculated purely from the *vendor's* zone.

**Schema.** No new address column for vendors (reused `store_address`) or region/zone machinery (reused `delivery_regions`/`delivery_zones` from the feature above) — this is additive wiring, not new infrastructure. `store_address`'s comment was updated to note it's now shared by `delivery_company` accounts too, as their company/home-base address, rather than renaming the column (renaming would touch every existing `storeAddress` reference for no behavior change). `users.delivery_zone_id` — previously Super-Admin-assigned only, restricted to `role = 'vendor'` — is now also self-service for both `vendor` and `delivery_company` (the Super Admin's own assignment route is untouched and can still override it). `saved_addresses` gained a nullable `zone_id` FK, since a customer's zone lives per saved address (they can have several, in different zones), not on the user the way a vendor/delivery company's single zone does.

**A real ordering bug caught before shipping.** The first version of the `saved_addresses.zone_id` migration was placed right next to the rest of `saved_addresses`'s definition, near the top of `schema.sql` — but `delivery_zones` (what it references) isn't created until much further down the file. On a truly fresh database, `db.init()` sends the entire `schema.sql` file as one batched query (`pool.query(schema)`), and Postgres implicitly wraps a multi-statement batch like that in one transaction: a FK referencing a table that doesn't exist yet fails and rolls back the *entire* batch, not just that one statement — which would have crashed every fresh deploy of this app at boot. This was only caught by testing against a truly fresh (never-before-migrated) local Postgres database with the migration sent as one batched statement (matching `pool.query`'s actual behavior) rather than via `psql -f`, which — unlike `pool.query` — runs each statement independently and would have hidden the bug entirely (a second run "succeeded" only because the first run's later, correctly-ordered `CREATE TABLE delivery_zones` had already run by then). Fixed by moving the `zone_id` column addition to right after `delivery_zones` itself is created.

**Backend.** `db.setSelfDeliveryZone(userId, zoneId)` is the self-service counterpart to the Super Admin's existing (untouched) `setVendorDeliveryZone` — same shape, but scoped to `role IN ('vendor', 'delivery_company')` instead of `'vendor'` only. `createUser` now accepts optional `storeAddress`/`deliveryZoneId` at creation time, so registration can set them in one insert. `createSavedAddress`/`updateSavedAddress` accept an optional `zoneId`, following the same "undefined means don't touch this field" convention `updateUserProfile` already used for `storeAddress`. `PUT /api/me/profile` now allows `storeAddress` for `delivery_company` too (previously vendor-only) and accepts a self-service `deliveryZoneId`, validated against a real zone via a new shared `validateOptionalZoneId()` helper (also used by the three registration routes and the saved-addresses routes). `POST /api/auth/register` (customer) accepts an optional `address` + `deliveryZoneId`; if an address was given, it becomes the customer's first, default saved address, created as a best-effort follow-up write that never fails the account creation that already succeeded. `POST /api/auth/register-vendor` and `POST /api/auth/register-delivery-company` accept an optional `deliveryZoneId` (and, for delivery company, `storeAddress`, since that role never had one to collect before). `publicUserShape()` and every route returning a user object (`GET /api/me`, login, 2FA verify, both registration routes) now include `deliveryZoneId`, which they never did before this — meaning a vendor's own session previously never carried their own admin-assigned zone at all, only the Super Admin's separate Vendors-table view did.

**Frontend — a reusable Zone Search Picker.** Built as one shared widget (`zoneSearchPickerHtml()`, `wireZoneSearchPicker()`, `selectZoneForPicker()`/`clearZoneForPicker()`/`presetZoneForPicker()`) rather than seven separate implementations — a debounced text input filters `deliveryZonesCache`/`deliveryRegionsCache` (already loaded app-wide pre-login by `loadPublicConfig()`, so it works at registration too) by zone code, zone name, or region name, and shows a clickable results list, modeled on the same pattern the Support Inbox "New Message" recipient picker already used, rather than a long native `<select>`. Picking a result shows a "you picked this" chip with a Change link back to search. Each of the seven usage sites is just a placeholder `<div id="${prefix}-zone-picker-container">` in the static HTML, filled in and wired once at boot by `initZoneSearchPickers()`.

**Frontend — wired into all seven places.** Registration: `register-vendor-fields` (zone picker only — store address stays a Settings-only field, as before), `register-delivery-company-fields` (new Company Address field + zone picker, since this role had neither before), `register-customer-fields` (new Delivery Address field + zone picker, becoming their first saved address). Settings: `vendor-settings-form` (zone picker next to the existing Store Physical Address field, pre-filled from `currentUser.deliveryZoneId` when the tab opens), `dc-settings-form` (new Company Address field + zone picker, same pre-fill treatment). Saved Addresses: both the marketplace app's `address-form` and the standalone delivery-customer app's `dcust-address-form` (two separate UIs sharing the same `/api/addresses` backend, per the existing app architecture) gained the picker, pre-filled from the address's own `zoneId` when editing, and included in both the create and update payloads.

**Verified.** `node --check` passed on `server.js` and `db.js`, and on the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline. Against a real disposable local Postgres instance: the ordering-bug fix was confirmed by running `schema.sql` as one batched statement (matching `pool.query`'s real behavior) against a truly fresh database — clean on the first run and idempotent on a second; a delivery company was seeded with both a home-base address and a self-service zone in one insert, confirmed queryable; a customer's saved address was seeded with a zone tag and confirmed joinable back to its region/zone; deleting a zone was confirmed to fall both a delivery company's `delivery_zone_id` and a saved address's `zone_id` back to null via their FKs, never leaving either pointing at a deleted zone. A Playwright pass confirmed, across registration for all three roles: the zone search box filters correctly by code/name/region, clicking a result selects it and shows the correct chip text, and Change correctly clears the selection back to an empty search box; vendor Settings pre-fills the picker from the vendor's already-assigned zone and a changed selection is sent correctly on save; a customer's saved-address form correctly includes the picked zone in its create payload. Zero page exceptions throughout. The full accumulated regression suite was re-run afterward and still passes clean.

## Available Orders + Recent Deliveries moved off the Super Admin Dashboard into their own "Orders" section

**Requested:** "Please remove this from Super Admin dashboard and give it his own section," pointing at a screenshot of the Available Orders queue (search/filter bar, + New Order, Select All, Delete Selected, order cards) and the Recent Deliveries panel next to it — both of which sat inline on the combined Dashboard/Overview page Super Admin lands on after login, mixed in with Platform Overview stats (Total Vendors, Pending Applications, Premium Subscribers, etc.). Scoped via two clarifying questions before touching any code: only Available Orders + Recent Deliveries move at first (Today's Snapshot, the Live Operations spotlight, Delivery Status cards, and the Revenue/Order Status charts all stayed on the Dashboard initially); the new home is a dedicated sidebar/More-sheet nav item, "Orders," matching how Vendors, Staff Accounts, Payouts, and Disputes already work — a modal opened from the nav, not a second full-page view. A same-day follow-up request — "Remove Revenue Overview (This Week) and Order Status (This Week) to the Orders section too" — extended the move to include those two charts as well; the section below describes the final combined shape, not the intermediate one.

**Why this had to be role-aware, not a simple move.** `#admin-operational-view` — the block containing Available Orders and Recent Deliveries — is shared markup between Manage Agent and Super Admin: it's Manage Agent's entire operational home dashboard (unaffected by this request, and not to be changed), while for Super Admin it renders alongside the Platform Overview stats block on the same combined page. Statically moving the HTML into a modal would have pulled it out of Manage Agent's dashboard too, which nobody asked for and would have broken their main workflow. Duplicating the markup instead (one copy inline, one copy in the modal) was ruled out — this app already treats duplicate static element ids as a shipped-bug class (see the div-balance/duplicate-id checks every feature in this README gets verified against), and `#orders-grouped-delivery`/`#recent-deliveries-container`/etc. are read by `document.getElementById` throughout the order-loading and rendering code, which only ever finds the first match.

**The fix: relocate the live DOM node by role, not clone it.** `#available-orders-section` and the Recent Deliveries `<section>` were wrapped in one container, `#admin-orders-block`; the Revenue Overview / Order Status charts (previously nested inside that same two-column layout, right under Available Orders) were pulled out into their own full-width `.admin-charts-row` as a sibling of it, so the two-column grid isn't stretched by the charts. Both `#admin-orders-block` and `.admin-charts-row` then got wrapped together in one outer container, `#admin-orders-section-content` — this is the actual unit that moves — left in its original inline position in the static HTML (Manage Agent's default, unchanged). A new empty marker div, `#admin-orders-inline-anchor`, sits right after it purely to remember where it goes back to. At the same point login already computes `isSuperAdmin` and toggles every other role-gated nav item, a new block does one of two things, idempotently (safe to re-run on every login): for Super Admin, `orders-modal-body.appendChild(ordersBlock)` — moving the actual live node (not `innerHTML`-cloning it) into the new `#orders-modal`, so every id, event listener, and cached DOM reference (`orders-grouped-delivery`, `recent-deliveries-container`, the order search/filter inputs, Select All/Delete Selected, the two chart canvases and their Chart.js instances) keeps working exactly as before, just inside a modal instead of inline; for Manage Agent, `ordersInlineAnchor.parentElement.insertBefore(ordersBlock, ordersInlineAnchor)` — restoring it to its original inline spot if it had been moved out (e.g. a Super Admin account was demoted, or the same browser session re-enters as a different role). Because the charts ride along inside the same moved unit rather than being tracked separately, the follow-up request to include them needed no new relocation logic at all — only widening what one existing wrapper contains.

**New "Orders" entry point.** A sidebar item (`open-orders-btn`, in the existing "Delivery Operations" nav group, right above Order History) and a matching More-sheet item (`admin-more-orders`) — both Super Admin only, following the exact same show/hide-by-role and "close the More sheet, then click the target nav button" pattern already used for Staff Accounts/Payouts/Disputes — open `#orders-modal`, a standard `.modal-overlay` styled like every other admin modal in this app. Two existing shortcuts that used to scroll to the inline section were updated to open this modal for Super Admin instead: the Live Operations spotlight's "View Available Orders →" button (shared with Manage Agent, so it still scrolls inline for that role and only opens the modal when `currentUser.role === 'super_admin'`) and the Super-Admin-only "Delivery Operations" Quick Action card.

**Verified.** `node --check` passed on the extracted client script (this was a frontend-only change — no `server.js`/`db.js`/`schema.sql` touched, on either the initial version or the charts follow-up). Duplicate-static-id and `<div>`-balance checks held at baseline both times. The actual relocation mechanism — the riskiest part of this change — was verified in isolation with a standalone Playwright test: the exact HTML skeleton and JS relocation logic were loaded in a real browser and driven directly, confirming the block starts inline, moves into the modal body when Super Admin is applied, stays intact and idempotent when the same role is re-applied, moves back to its exact original position (immediately before the anchor) when Manage Agent is applied, survives a full round trip back to Super Admin, and never loses its content along the way. The charts follow-up re-ran this same isolated test against the widened wrapper (`#admin-orders-section-content` now containing both `#admin-orders-block` and `.admin-charts-row`) and separately confirmed, by loading the real edited `index.html` in a browser, that `.admin-charts-row` and both chart canvases are correctly nested inside the new wrapper and that `#admin-orders-inline-anchor` sits immediately after it as a sibling, not a child. (A full end-to-end login-based Playwright pass against the running server, the usual verification step for this session, wasn't possible for either version — this sandbox's npm registry access was unavailable, so `server/node_modules` couldn't be installed — but the schema was unaffected and confirmed unchanged, and the isolated DOM tests cover the one genuinely new mechanism this change introduces.)

## Customer Address and Zone (Home Base) management moved into Settings; Addresses removed from the nav bar

**Requested:** "Please work on customers Address and Zone (Home Base) import in settings, and remove Addresses from the bar." Scoped via two clarifying questions before touching any code, since the same session had already learned this user's "import" doesn't always mean a literal bulk-paste import (the earlier Zone Search Picker feature was requested in nearly identical wording — "import their addresses by searching their Regions/Zones" — and turned out to mean a search picker, not a bulk import): confirmed this meant moving address management into Settings entirely (not just adding a Home Base field alongside the existing, separately-located address book), and confirmed it should apply to both customer-facing surfaces — the Marketplace app and the standalone delivery-customer app — for consistency, not just whichever one had Addresses sitting in an actual nav bar.

**What "the bar" turned out to include.** Investigation found three separate places customers could reach the old standalone Addresses screen, not one: the Marketplace desktop sidebar (`.desktop-sidebar-item[data-mp-tab="addresses"]` — a genuine bar item, missed in the first pass of research and only caught by a full-file grep for `Addresses` before writing any code), the Marketplace mobile Account menu (`#mp-account-menu-addresses`, a menu row rather than a bar item), and the standalone delivery-customer app's desktop sidebar (`#dcust-nav-addresses`) plus its mobile bottom nav (`#dcust-mnav-addresses`). All five were removed. Both apps still have a "Settings" nav entry left in place (`#mp-account-menu-settings`, `#dcust-nav-settings`, plus delivery-customer's mobile More sheet already had `#dcust-more-settings`), so nothing about the path to Settings itself changed — only the now-redundant separate Addresses destination is gone.

**New Home Base zone.** Both roles' Settings forms gained a self-service "Home Base" zone picker — the same Zone Search Picker widget already used by vendors and delivery companies, reusing `zoneSearchPickerHtml()`/`wireZoneSearchPicker()`/`presetZoneForPicker()` with no changes to that widget itself. This is a single preferred zone stored on the user row (`users.delivery_zone_id`), separate from the per-address zone a customer can also set on each individual entry in their saved-addresses book — one customer can have several addresses in different zones, but only one Home Base. `db.setSelfDeliveryZone()`'s role check (`vendor`, `delivery_company`) gained `sender`, and `PUT /api/me/profile`'s zone-setting permission (`canSelfSetZone`) now includes `sender` too — split out from a new, narrower `canSelfSetStoreAddress` flag so `storeAddress` (which customers don't have — they have a whole saved-addresses book instead) stays exactly as vendor/delivery_company-only as before. Customer registration was deliberately left untouched — a zone picked at signup still only tags the customer's first saved address, exactly as the earlier Zone Search Picker feature built it; Home Base starts unset and is set later from Settings, keeping this change scoped to exactly what was asked.

**Saved Addresses relocated, not rebuilt.** The entire existing address book — list, "+ Add", inline add/edit form with its own per-address zone picker, edit/delete/set-default — was physically moved from its old standalone location into each app's Settings screen, with every element id, function name, and API call left completely unchanged (`loadSavedAddresses()`/`renderSavedAddressesList()`/`openAddressForm()`/`deleteSavedAddress()`/`setDefaultSavedAddress()` for Marketplace; the delivery-customer equivalents, renamed only where the old function was named after the modal it used to open — `openDcustAddressesModal()` became `loadDcustAddressesList()` since there's no longer a separate modal to open, just a list to (re)render inside Settings). Marketplace's `MP_TAB_VIEWS` array and `setMarketplaceTab()` dropped the `'addresses'` case entirely; opening the `'settings'` tab now does everything the old `'addresses'` tab used to (loads the list, resets the add/edit form) in addition to what it already did. The delivery-customer app's `openDcustSettingsModal()` picked up the same responsibility. A location-pill shortcut ("Add your delivery address," shown when a delivery-customer guest has no saved address yet) that used to click straight through to the old Addresses nav button was repointed to open Settings and jump straight to the Add Address form in one call, so the one-click shortcut still lands exactly where it used to.

**Verified.** `node --check` passed on `server.js`, `db.js`, and the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline. Against a real disposable local Postgres instance: `schema.sql` ran clean on a fresh database and was confirmed idempotent on a second run; a `sender` row was seeded and `setSelfDeliveryZone`'s exact updated SQL was run directly, confirming a customer's Home Base zone now sets successfully (previously blocked by the role check) and correctly falls back to null when the zone is deleted, via the same `ON DELETE SET NULL` behavior vendors and delivery companies already relied on. A standalone Playwright pass loaded the real, edited `index.html` as a static page (no backend — this sandbox's npm registry access was unavailable this session, same limitation as the feature above) and confirmed structurally: all five old Addresses nav/menu entries and the old standalone Addresses screen/modal are gone from the DOM; the moved Saved Addresses list, Add button, and inline form are all present and correctly nested inside each app's Settings container under their original ids; both new Home Base zone pickers rendered their full search-input/hidden-id widget markup correctly via `initZoneSearchPickers()`; `MP_TAB_VIEWS` no longer contains `'addresses'`. Zero JavaScript exceptions during page load — only expected 404s from the missing backend in this static-only test. The full accumulated regression suite wasn't re-run against a live server this time for the same npm-registry reason noted above. `node --check` passed on the extracted client script (this was a frontend-only change — no `server.js`/`db.js`/`schema.sql` touched). Duplicate-static-id and `<div>`-balance checks held at baseline. The actual relocation mechanism — the riskiest part of this change — was verified in isolation with a standalone Playwright test: the exact HTML skeleton and JS relocation logic were loaded in a real browser and driven directly, confirming the block starts inline, moves into the modal body when Super Admin is applied, stays intact and idempotent when the same role is re-applied, moves back to its exact original position (immediately before the anchor) when Manage Agent is applied, survives a full round trip back to Super Admin, and never loses its content along the way. (A full end-to-end login-based Playwright pass against the running server, the usual verification step for this session, wasn't possible this time — this sandbox's npm registry access was unavailable, so `server/node_modules` couldn't be installed — but the schema was unaffected and confirmed unchanged, and the isolated DOM test covers the one genuinely new mechanism this change introduces.)

## Vendors can request an order be voided out of revenue, for returns — reviewed by Super Admin in the existing Disputes queue

**Requested:** "I need venders to be able to send Super Admin request to delete Orders from history for correct revenues record because customers sometime send orders back and the payment record needs to be deleted from the system," alongside a screenshot of the Vendor Orders page (order cards with buyer, payment method, status pill, amount, "Send to Delivery"). The literal ask — delete rows from the database — was scoped down through two rounds of clarifying questions (the first round the user asked to have explained in plainer language before answering) into three confirmed decisions, quoted back verbatim from the user's own reply: **"Void it, keep it"** — no hard delete, ever; the order stays visible in order history exactly as before, just flagged so it stops counting toward revenue, "because remember, the delivery fees stays with the delivery companies" (an explicit added constraint: whatever a vendor's product revenue does, a delivery company's earnings for actually delivering the order must never move). **Fold into the existing Disputes queue** — Super Admin reviews these the same screen already used for customer complaints, not a new dedicated screen. **Everywhere** — once approved, the order's product revenue stops counting in the vendor's own sales totals, the Super Admin Payouts & Commission balance, and every platform-wide revenue figure, not just one narrow spot.

**Why "void it, keep it" was already the only consistent choice.** This app has never actually deleted a business record for a correction — `vendor_dismissed`, `vendor_cancel_requested`, and every dispute/return status already work by flagging, not deleting, so there's always a paper trail if a number ever looks wrong later. This feature follows the exact same shape: a new `purchases.excluded_from_revenue` boolean (plus `excluded_from_revenue_reason`/`_at` for the trail) that every revenue query now checks, set only once a Super Admin approves — the purchase row itself is never touched otherwise.

**Why the delivery-fee constraint needed no special-casing.** `purchases.total_amount` (a vendor's product revenue) was already a completely separate column from `delivery_fee`, and delivery-company revenue was already computed from a different table entirely (`orders.amount WHERE status = 'delivered'`, never `purchases`). Scoping every change to `purchases`-table queries alone, and never touching `orders`, satisfies "the delivery company keeps its fee" by construction — confirmed directly against a real Postgres instance (see Verified below) by voiding a purchase linked to a delivered order and checking the order's own delivery-revenue sum was completely unaffected.

**The request → review → void flow.** A vendor sees a "🗑️ Request Deletion" button on any of their own Orders-list purchases that currently counts toward revenue (`payment_method = 'cod' OR payment_status = 'successful'`) and isn't already voided or awaiting a decision (new `POST /api/vendor/purchases/:id/request-deletion`, ownership-checked, blocks a second open request the same way `POST /api/disputes` already blocks a duplicate customer complaint). That creates an ordinary `disputes` row — reusing the table entirely, not a new one — distinguished from a customer's own complaint by two new columns, `initiated_by` (`'customer'` default / `'vendor'`) and `vendor_id`; `customer_id` still gets set (to the purchase's real customer) so every existing customer-keyed query keeps working unchanged. The `category` CHECK was widened to add `'vendor_return'` (constraint name `disputes_category_check`, re-confirmed live against Postgres, same convention as every other CHECK rename in this app). It shows up automatically in Super Admin's existing Disputes modal — the category label itself reads "Vendor return / void request," and the resolve-dispute summary adds a small "Filed by vendor" badge — no second queue, exactly as requested. The existing resolve-dispute form gained a third decision alongside Refund/Reject: **Void — remove from revenue**, shown only when the dispute has a purchase attached (an order-only dispute is a Delivery Company matter and has nothing to void). Choosing it resolves the dispute (status `'resolved'`, no refund amount) and calls a new `db.excludePurchaseFromRevenue()`, which is the only thing that ever sets the flag — a vendor can request, but never approve, their own void.

**"Everywhere" — every revenue query updated.** `AND NOT excluded_from_revenue` (or the aliased equivalent) was added to every purchases-table revenue query in `db.js`: `getVendorSalesOverview`/`getVendorDailySales` (the vendor's own Sales Overview stat and chart), `getVendorCustomers` (per-customer totals), `getMarketplacePlatformStats`/`getBusinessOverviewStats` (Super Admin's platform-wide and Marketplace/Restaurant-split revenue), `getPayoutSummary`'s vendor-gross query specifically (its sibling `deliveryRevenue` query, a completely different table, was deliberately left untouched), and `getCommissionStatement`'s vendor branch (the downloadable per-period commission statement — a pre-existing, unrelated gap where this query had no payment-status filter at all was left as-is; only the new exclusion filter was added, to avoid changing unrelated historical numbers). `getPurchasesByVendor` (the Orders list itself) gained a `LEFT JOIN LATERAL` pulling this vendor's own most recent void request status, so the list can show "Deletion Requested" (pending) or "Voided (not counted)" instead of letting a vendor file a second request while one's still open.

**Verified.** `node --check` passed on `server.js`, `db.js`, and the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline (diff of 1, unchanged from every prior feature this session). Against a real disposable local Postgres instance: `schema.sql` ran clean on a fresh database; `\d disputes` confirmed the exact expected shape — `initiated_by`/`vendor_id` columns, `disputes_category_check` widened to include `'vendor_return'`, a new `disputes_initiated_by_check`, and the new `idx_disputes_vendor_id` partial index. The full request→approve→void flow was then run as raw SQL matching the new query text exactly: seeded a vendor with two $100/$50 purchases (gross $150), inserted an `initiated_by = 'vendor'` dispute against the $50 one, resolved it, and set `excluded_from_revenue = true` — after which the vendor-gross query correctly dropped to $100 and the purchase row was confirmed still present in the table (not deleted) with its reason and timestamp populated. Separately seeded a delivered order (`amount = $12`) linked to the now-voided purchase and confirmed the delivery-company-revenue query still summed the full $12 — proving the delivery-fee-preservation constraint holds by construction. The vendor-side deletion-request-status lateral join was run directly too, confirming it correctly reports `'resolved'` for the voided purchase and null for the untouched one. A standalone Playwright pass loaded the real, edited `index.html` as a static page and confirmed: the new resolve-dispute "Void" radio only appears for a dispute with a purchase attached and correctly hides the refund-amount field and shows the void explanation hint when selected; `disputeSummaryHtml` renders the "Filed by vendor" badge for a vendor-initiated dispute; a resolved-and-voided dispute's read-only view shows a distinct "Voided — excluded from revenue" badge instead of an ambiguous "Resolved"; and `vendorOrderRowHtml` renders the "Request Deletion" button, the "Voided (not counted)" badge, and the "Deletion Requested" badge in exactly the right one of those three states depending on the purchase's flags, with the button never appearing for a purchase that doesn't yet count toward revenue (e.g. a still-pending Mobile Money payment). Zero JavaScript exceptions in every pass — only expected 404s from the missing backend in this static-only test, the same npm-registry limitation as every other feature this session (`server/node_modules` couldn't be installed, so no live-server end-to-end pass was possible).

## Live bell/notification-center signal on every request or message that reaches admins

**Requested:** "Please add a notification single on every requests/messages send to admins" — a follow-up to the vendor deletion-request feature above, once it became clear that request (like several other existing admin-facing queues) updated its data silently but never actually notified anyone. Scoped via one clarifying question: cover every existing "this needs a Super Admin/admin to look at it" queue or inbox — disputes (customer complaints and the new vendor void requests), vendor cancellation requests, Mobile Money payment confirmations, and Support Inbox chat messages — **plus** vendor/delivery company sign-up applications, which the user picked from the broader of two scope options offered.

**What "notification" means here.** This app already has a real in-app notification center — the bell icon in the admin top bar, backed by `sendLocalNotification()` → `recordInAppNotification()`, which appends to a session-only `notificationLog`, drives the unread badge count, and (where the browser has granted permission) also fires a native OS notification via `Platform.notify()`. It was already used for order events (new order placed, order accepted/cancelled/updated) and a couple of others (expenses, a customer's own dispute being resolved) — but nothing that lands specifically in an *admin's* queue ever called it. This feature doesn't invent a new notification mechanism; it just wires that existing one up to the events admins were already silently missing.

**The gap, event by event.** `marketplace_payment:new` (fired on a new Mobile Money checkout and, confusingly, also reused for vendor cancellation requests) already had a live socket handler — but it only silently called `loadMarketplacePaymentsPendingQueue()` to refresh the Payouts panel's badge/queue if it happened to be open; no bell entry, ever. `dispute:new` didn't exist at all — neither a customer filing a complaint (`POST /api/disputes`) nor a vendor's void/deletion request (`POST /api/vendor/purchases/:id/request-deletion`, added just before this feature) emitted anything to admins; the Disputes nav badge only ever updated when someone happened to open the Disputes modal. `support:new` (a customer/vendor/delivery-company message to the live support chat) already reached admins over the socket, but the handler only live-appended or refreshed the thread list when the Support Inbox modal was already open — closed, it did nothing, not even a silent badge refresh. New vendor/delivery-company applications had a real notification path (`notifyNewVendorApplication()`, an email via `notify.js`) but nothing in-app — an online Super Admin with SMTP unconfigured (the common case in this sandbox/dev setup) had zero signal.

**The fix — five socket touchpoints, one shared pattern.** Server-side: `POST /api/disputes` and `POST /api/vendor/purchases/:id/request-deletion` now both `io.to('admins').emit('dispute:new', dispute)`; the three existing `marketplace_payment:new` emit sites gained a `kind` field (`'new_payment'` for a checkout, `'cancel_request'` for a vendor's cancellation ask) so the client can word the notification correctly without an extra round trip; `POST /api/auth/register-vendor` and `POST /api/auth/register-delivery-company` each gained a new `io.to('admins').emit('vendor_application:new', {...})` alongside the existing email attempt. Client-side, each handler follows the same shape as the order-event handlers that already worked this way: check the current user actually has access to that queue (`super_admin` for disputes/payments/applications; `super_admin` or an `admin` who hasn't had `support_inbox` restricted, mirroring the exact same `myDisabledFeatures` check that already hides the Support Inbox nav button for a restricted Manage Agent — added because the *previous* support-message handler never needed this check, since it only ever ran when the inbox was already open, which a restricted admin could never do; the new "refresh even when closed" behavior needed the same guard added explicitly), do whatever silent data refresh already existed (or, for disputes, a new one — refetching `openCount` so the nav badge stays live instead of only updating on next visit), and call `sendLocalNotification()` with a title that reflects what actually happened (`"New Mobile Money payment"` vs `"Cancellation requested"`; `"New dispute filed"` vs `"Vendor requested a void"`; `"New vendor application"` vs `"New delivery company application"`). The support-message handler specifically was restructured so the badge-refresh-and-notify path fires whenever a message is *from* a user, independent of whether the Inbox modal happens to be open — previously that whole branch was gated behind `inboxOpen`, so a closed Inbox meant no live badge update at all, not just no bell entry.

**Verified.** `node --check` passed on `server.js` and the extracted client script after every edit. Duplicate-static-id and `<div>`-balance checks held at baseline. Every one of the five new/changed server-side emit sites was confirmed by direct grep to carry the right event name and payload shape. Client-side, since a real Socket.io connection isn't available in this sandbox (same npm-registry limitation as every other feature this session), the handlers were tested by stubbing the global `io()` factory to capture the real `socket.on(event, handler)` registrations `connectSocket()` makes, then invoking each captured handler directly with representative payloads against the real, unmodified page code (no reimplementation of the logic in the test) — confirming: a new Mobile Money payment and a cancellation request each produce the correctly-worded bell notification and refresh the Payouts queue, and both are silently ignored for a non-admin role; a vendor-initiated dispute and a customer-initiated one each produce a correctly-worded notification and refetch the Disputes nav badge's open count; a vendor application and a delivery-company application each produce the correctly role-worded notification; a support message from a user notifies and refreshes the Support Inbox list/badge for an admin who has access, is completely silent for one whose `support_inbox` feature has been restricted, and — critically — does *not* re-notify support staff about their own outgoing reply being echoed back (`senderRole: 'support'`). All 13 assertions passed with zero JavaScript exceptions.

## Auto-logout after 30 minutes of inactivity, every account and every app

**Requested:** "Please put a auto logout on every account after 5 minutes of being inactive." Investigated the existing session model first (there wasn't any inactivity concept at all — sessions previously only ended via a 30-day JWT expiry, an explicit Logout, "Logout All Devices," or a Super Admin disabling/changing a role) and confirmed no role has a legitimate reason to sit idle-but-still-need-the-session (no live GPS/kiosk-style background activity exists anywhere — a schema comment on `agents` explicitly disclaims that pattern, since delivery agents aren't logged-in accounts at all in this app). Understanding was confirmed with the user before writing any code, then three implementation details — whether to warn before logging out, whether background events like incoming orders should count as "activity," and whether multiple open tabs should share one idle clock — were settled via a clarifying question: warn 30 seconds before the cutoff, count only real interaction (not live socket events), and sync across tabs. The user also changed the timeout itself from the originally-requested 5 minutes to 30.

**One shared timer for the whole app.** Since every role (customer, vendor, delivery company, admin, super admin) and every "app" (Marketplace, Delivery, Admin/Super-Admin console) lives in this one frontend file, this is a single mechanism rather than five separate ones — started in `enterApp()` (the one function every login and session-restore path already funnels through) and stopped in `clearAuth()` (the one function every logout path already funnels through), the same choke points the existing socket-connection lifecycle already uses.

**What counts as "inactive."** Five real interaction events — `mousemove`, `mousedown`, `keydown`, `touchstart`, `scroll` — reset the clock; a `setInterval` polls every second to compare elapsed time against a 30-minute (`IDLE_TIMEOUT_MS`) cutoff. A busy live feed of incoming orders or chat messages over the socket deliberately does *not* reset it — someone who's actually stepped away still gets logged out even while their screen keeps updating on its own, per the user's explicit choice.

**Synced across tabs via a user-scoped localStorage heartbeat.** Real interaction writes the current timestamp to `localStorage` (throttled to once/second — `mousemove` fires far more often than that) under a key that includes the logged-in user's id (`onlib_idle_last_activity:<userId>`), not a single shared key — so activity in one tab of the same account keeps every other tab of that account alive (the whole point of "sync across tabs"), while two *different* accounts open in the same browser at once never reset each other's clocks. A `storage` event listener lets a tab currently showing the warning modal hide it immediately the moment another tab records activity, rather than waiting for its own next poll tick. This deliberately uses real browser `localStorage`, not the app's `Platform.storage` abstraction — that abstraction may route to native storage on a future Capacitor build (confirmed not wired in yet) and isn't guaranteed to behave the same way across tabs, where this feature specifically needs it to.

**The warning.** At 30 seconds remaining, a new global modal (`#idle-warning-modal`, alongside `#toast-container` — outside any one app's markup, since it applies everywhere) appears with a live countdown and two buttons: "Stay Signed In" (explicitly resets the clock and closes the modal immediately, though in practice just clicking anywhere — including the modal itself — already does this via the same document-level listeners) and "Log Out Now" (skips the wait). If nothing happens, the countdown reaching zero calls the same `clearAuth()` → `showAppChooser()` sequence every manual logout button already uses, then shows a toast explaining why ("You've been logged out due to inactivity").

**Verified.** `node --check` passed on the extracted client script and `server.js` (this was a frontend-only change). Duplicate-static-id and `<div>`-balance checks held at baseline. Since real wall-clock testing of a 30-minute timer isn't practical, the underlying mechanism was exercised directly and deterministically: a standalone Playwright pass loaded the real, edited `index.html`, called the actual `startIdleTracking()`/`checkIdleTimeout()`/`stayLoggedIn()`/`stopIdleTracking()` functions, and manipulated only the localStorage timestamp they read (never the code itself) to simulate elapsed time — confirming: no warning or logout fires immediately after starting; the heartbeat key is correctly scoped by user id; the warning appears with an accurate countdown once inside the 30-second window and disappears the instant real activity (a simulated `keydown`) is recorded, with the stored timestamp genuinely refreshed; the "Stay Signed In" and "Log Out Now" buttons work identically whether called as functions or clicked for real in the DOM (`clearAuth`/`showAppChooser` were confirmed called exactly once for the latter); reaching the full timeout calls `clearAuth()`, `showAppChooser()`, and shows the inactivity toast; a simulated `storage` event from another "tab" hides an open warning immediately; switching `currentUser` to a different id produces a distinct storage key with no stale data inherited from the previous user; and `stopIdleTracking()` clears the interval and hides any open warning. 19 assertions across two passes, zero JavaScript exceptions.

## Rider Payout % — delivery companies can set a commission rate per agent, reflected in the Monthly Report PDF

**Requested:** "Every delivery companies Agent ride on a percentage base... can you set a area where delivery companies can be able to set riders percentage for monthly report (PDF File)." Investigated the `agents` table, how orders attribute to an agent, and the existing Monthly Report PDF's agent breakdown before writing any code, then two open design questions were settled via a clarifying question, both answered with the recommended option: the percentage applies to the delivery fee only — `orders.amount` — not to customer tips (`orders.tip_amount` is already documented in `schema.sql` as "not commissionable," which independently confirmed this was the right split before the question was even answered); and only the delivery company that owns an agent can set or edit that agent's percentage — Super Admin has no control over it, even though Super Admin can otherwise manage any company's Fleet Directory.

**A new nullable column, not a required one.** `agents.payout_percent NUMERIC(5,2)`, CHECK'd to `NULL` or `0–100`, added to schema.sql. `NULL` (the default — every existing agent) means "no payout percentage set" — the report still lists that rider's orders and their dollar amount, it just can't compute a payout for them, and doesn't guess one.

**Set from the Fleet Directory's own agent form.** A "Payout %" field was added to the delivery company's existing add/edit agent form (`#dc-agent-form`) — the same numeric-input pattern (`type="number" min="0" max="100" step="0.01"`) already used for the platform's commission-rate fields, rather than the older `prompt()`-based pattern used elsewhere. It's optional; leaving it blank on create stores `NULL`, leaving it unchanged on edit does nothing (mirroring the existing `deliveryCompanyId` "undefined = leave unchanged" convention already used by `db.updateAgent`), and clearing it back to blank explicitly stores `NULL` again. This field doesn't exist on the Super Admin's parallel agent-management UI — enforced server-side too: the `agent:create`/`agent:update` socket handlers only ever honor a `payoutPercent` value when `socket.user.role === 'delivery_company'`; any value sent by an admin/staff account is silently ignored, the same defense-in-depth already applied to `deliveryCompanyId` reassignment being admin-only.

**Grouped by agent, not by name, for the money math.** The Monthly Report PDF's existing "Agent Summary" table grouped orders by the free-text `acceptedBy` name string — fine for a simple headcount, but schema.sql's own comment on `orders.agent_id` already explains why that's unsafe once real money is involved ("two agents... can share a name, so it was never safe to key a rating off it"). The table now groups by `orders.agentId` (the real FK, populated on every order accepted since it was added) when present, and looks up that agent's *current* Payout % live from the company's roster — so a rate change updates future reports immediately without needing to touch historical order rows. Orders from before `agentId` existed, or whose agent has since been removed from the roster, fall into a legacy/unattributable bucket: still counted and totaled by name, just never assigned a payout, following the exact same "not reliably attributable, so don't guess" precedent this codebase already established for agent ratings. Two new columns (`%`, `Payout`) were added to the existing Agent/Orders/Amount table, plus a "Total Rider Payouts" summary line (sum of every row that *could* be calculated) and, only when at least one row couldn't be, a one-line disclaimer explaining why.

**Verified.** `schema.sql` was applied against a real local Postgres database end to end, confirming the new `payout_percent` column, its `NULL`-or-`0–100` CHECK constraint (an out-of-range insert was rejected; `NULL` and in-range values were accepted), and — since `db.js` couldn't be loaded directly against Postgres in this sandbox (no `pg` package available, same npm-registry limitation as the rest of this session) — the exact parameterized SQL `updateAgent()` builds for all four undefined/defined combinations of `deliveryCompanyId`/`payoutPercent` was extracted and hand-verified against the real database, confirming: editing name/phone alone leaves a previously-set percentage untouched, setting a new percentage overwrites it, explicitly clearing it stores `NULL`, and a subsequent name-only edit doesn't resurrect the cleared value. `node --check` passed on `server.js` and `db.js` directly, and on the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline. The PDF's payout math itself — the highest-risk part, since it's money — was tested by extracting the real, unmodified `generateDeliveryCompanyMonthlyReportPDF()` function (and its real helper functions) out of `index.html` and running it against synthetic orders covering all four cases at once (an agent with a percentage set across two orders, an agent with no percentage set, an order whose agent has since been removed from the roster, and a legacy order with no `agentId` at all), with `jsPDF` stubbed to record every rendered line: confirmed the two-order agent's amount summed correctly ($150.00), its percentage rendered (70%), its payout computed correctly ($105.00 = $150 × 70%), all three unattributable cases rendered `N/A` rather than a guessed number, the "Total Rider Payouts" line summed only the one calculable payout, and the disclaimer line appeared. A separate Playwright pass confirmed the new "Payout %" input renders in the real page with the correct `type`/`min`/`max` attributes. 10 payout-math assertions plus 3 form-field assertions, all passing.

## Fix: ONLib Delivery's mobile hamburger button overlapped the notification bell for logged-in customers

**Requested:** a screen recording showing that, on a phone, opening a store from the ONLib Delivery dashboard (e.g. "Girlee Fashion") revealed a hamburger icon pinned to the top-right corner, overlapping the notification bell — "remove the upper hamburger icon, it is causing overlap issues."

**Root cause — an inline style silently defeating an already-correct CSS fix.** The stylesheet already had the right rule: `#delivery-customer-app .dcust-mobile-topbar .sidebar-toggle-btn { display: none; }`, inside the mobile `@media (max-width: 1023px)` block, with a comment explaining exactly why — "The hamburger opens a sidebar that's always display:none below the desktop breakpoint anyway — dead weight here." But `setDeliveryHeaderState()`, the function that runs on every login and every guest→customer transition, unconditionally set `dcust-sidebar-toggle-btn.style.display = 'flex'` for any logged-in customer, at any screen width. An inline style always wins over a stylesheet rule regardless of specificity or media queries, so that one line silently overrode the correct CSS on every phone, every time — a guest browsing (no inline override applied) never saw the bug, which is why it only appeared after logging in or landing in "customer" mode. Once forced visible, the button's own base rule (`position: fixed`, a leftover from `#delivery-app`'s different sidebar design, where this same button sits fixed beside a real always-visible sidebar) pinned it to a fixed screen position that happened to collide with the notification bell sharing that same top-right corner of `.dcust-mobile-topbar`, rather than actually reflecting the responsive `display: none` the stylesheet had already correctly written.

**The fix.** One function, `setDeliveryHeaderState()`, changed to match the exact pattern already used two lines above it for the account sidebar itself: instead of force-setting `display: 'flex'` inline for a customer, it now clears the inline override (`style.removeProperty('display')`) and lets the existing, already-correct responsive CSS decide — hidden below 1024px, visible (and functional, toggling the real desktop sidebar) at or above it. Guests still get an explicit `display: 'none'`, unchanged. No CSS was touched — the stylesheet's fix was already right; the bug was JavaScript overriding it.

**Verified.** `node --check` passed on the extracted client script (frontend-only change). Duplicate-static-id and `<div>`-balance checks held at baseline. Since no live backend is available in this sandbox, the exact bug was reproduced end-to-end first — a Playwright pass loaded the real, unmodified `index.html` at a 384×832 mobile viewport with `/api/*` calls mocked, called the real `enterDeliveryMode('customer')`, and clicked through to a "Girlee Fashion"-equivalent store exactly like the video — confirming the toggle button rendered `display: flex`, `position: fixed`, sitting directly on top of the notification bell (visually confirmed in a cropped screenshot: the hamburger's three lines were visible bleeding through the bell icon's circular background). The same flow re-run against the fixed code confirmed `display: none` at every point — dashboard, after switching to the Grocery tab, and after opening the store — with a screenshot matching the clean reference layout (logo, bell, no hamburger) the request was compared against. A separate pass confirmed no regression at desktop width (1440px): the toggle still resolves to `display: flex` and the sidebar to `display: flex` for a logged-in customer there, since the fix only changes what happens below the existing 1024px breakpoint. A third pass confirmed the guest-mode path (never affected by the bug) still renders identically.

## Fix: mobile store page header rearranged — Cart next to Login/Sign Up, above the store's own Back button

**Requested:** a marked-up copy of the same "Girlee Fashion" reference screenshot from the hamburger fix above, circling the exact arrangement wanted: "Cart(0)" sitting directly above the store page's "← Back" button, and directly next to "Login / Sign Up" on the same row.

**What was wrong.** The Cart button lived inside the store/restaurant-menu view itself (`#dcust-view-restaurant-menu`), in a row alongside that view's own "← Back" button and the vendor name. On mobile, where the topbar above it independently wraps "Back to service selector" and "Login / Sign Up" onto their own stacked lines, Cart had no way to end up next to Login/Sign Up — it's a completely different flex container one row below. The two rows just happened to stack in whatever order each one wrapped in on its own, which after the hamburger fix put Cart below "← Back" instead of above it, and never next to Login/Sign Up at all.

**The fix.** Moved the Cart button itself out of the store view and into the persistent topbar row (`.dcust-topbar-row`), right before the Login/Sign Up button, with `setDeliveryCustomerView()` now explicitly showing it only while the restaurant-menu view is active (mirroring the same show/hide pattern already used for that view itself) — otherwise it stays hidden on the dashboard and My Orders, where a store isn't open. Being in the same flex container as Login/Sign Up means it can now genuinely share a row with it instead of just coincidentally landing nearby. Two small CSS additions in the existing `@media (max-width: 640px)` topbar-wrapping block make that grouping deterministic rather than lucky: "Back to service selector" gets `flex-basis: 100%` so it always claims its own line regardless of what else is in the row (previously, Cart being short enough to fit next to it was what caused the wrong grouping), and both Cart and Login/Sign Up get `margin-left: auto` so together they sit right-aligned on the line below, matching the reference. The store's own header row, now just "← Back" and the vendor name, is unchanged and still renders directly below.

**Verified.** `node --check` passed on the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline (the duplicate `id="dcust-restaurant-cart-btn"` the move would otherwise have created — one copy moved, the original not deleted — was caught by this check and fixed before the check passed clean). A Playwright pass reproduced the exact guest-browsing flow from the reference screenshot — mobile viewport, guest mode, Grocery tab, open "Girlee Fashion" — and the resulting screenshot matches the circled reference line for line: "Back to service selector" alone on row one, "Cart(0)" and "Login / Sign Up" together on row two, "← Back" and "Girlee Fashion" on row three. A second pass confirmed the Cart button's visibility is genuinely tied to the store view, not just usually hidden: `display: none` on the dashboard, `display: flex` only after opening a store, for both a guest and a logged-in customer (whose row renders Cart alone, right-aligned, since Login/Sign Up stays hidden for them — screenshot-checked, no visual breakage). A third pass confirmed no change at desktop width, where this row's mobile-only wrapping rules don't apply.

## Delivery fee now priced by the customer's zone (not the vendor's), plus editable name/phone at Checkout

**Requested:** "Delivery fees at Checkout shouldn't be inpletmented on Pickup Address (vendor/store) but Dropoff Address (customers)... please add customers import of name and contact at Checkout." Investigated the full checkout path first and confirmed with the user, before writing any code: the multi-vendor cart structure stays exactly as it is (one delivery fee line per vendor, summed) — what changes is what each vendor's fee is *based on*. The user's own example ("customer is at zone 1 and venders are at zone 14, let calculate the distance in between") turned into a second round of investigation, since this app has no geo/mapping data of any kind to compute a real distance from (confirmed — it's explicitly documented as an intentional omission in three places already in `schema.sql`/`server.js`, the same "honest substitute for real geolocation" reasoning behind zones existing at all). The user picked the option that fit that constraint: **the Super Admin explicitly prices every (vendor zone, customer zone) pair**, rather than the app inventing a distance calculation it has no data to back up.

**What actually changed.** Today, before this feature, a vendor's delivery fee was that vendor's own flat zone fee (`delivery_zones.fee`), completely independent of where the customer was. Now it's looked up from a new admin-managed table, `zone_pair_fees` — one row per `(vendor_zone_id, customer_zone_id, fee)` — with a documented, deliberate fallback: any pair the Super Admin hasn't priced yet, or a customer whose dropoff address has no zone attached at all, falls straight back to the vendor's own flat zone fee, i.e. exactly today's behavior. Nothing regresses to $0 or blocks checkout just because the admin hasn't filled in the whole matrix yet. Multi-vendor carts are unaffected in shape — still one fee per vendor group, summed — only the number each group's fee resolves to changed.

**Where the customer's zone comes from.** The existing Dropoff Address field at Checkout is a saved-address picker backed by a free-text fallback; saved addresses already carry an optional `zoneId` (added by an earlier feature, previously documented as *not* feeding into fee calculation at all — that comment is now superseded). Picking a saved address with a zone set now populates a hidden `#checkout-dropoff-zone-id` field that flows into the fee lookup; typing a new address by hand (no zone information available) clears it, which is exactly the "no resolvable zone" fallback case above — same flat-fee behavior a customer got before this feature existed. The Cart Modal's own delivery-fee line, shown before Checkout is even open (so before any dropoff is known), is now explicitly labeled "(estimated)" rather than silently showing a number that might not match what Checkout ends up charging.

**Editable Name/Phone, pre-filled but not locked.** Two new fields on the Checkout form, pre-filled from the logged-in account (`currentUser.businessName`/`.phone`) but editable per order — so, for example, an order can be placed as a gift for someone else without touching the account itself. `orders.sender_phone` is new (`sender_name` already existed); both flow through `db.checkout()` exactly like the existing pickup/dropoff addresses do, including the Mobile Money "pending" path — a manual Mobile Money order doesn't get its real delivery order created until a Super Admin confirms payment, so the typed name/phone are stashed on the purchase (`pending_sender_name`/`pending_sender_phone`, mirroring the pre-existing `pending_pickup_address`/`pending_dropoff_address` pattern exactly) and only promoted onto the real order at confirmation time — otherwise a customer's typed name would have silently reverted to the account's own name the moment a Mobile Money payment was confirmed, which is what would have happened without this fix since the deferred order-creation path only ever looked up the account's `business_name` fresh. The delivery agent's own Order Details views (accept screen, order card) now show the contact phone as a tappable `tel:` link when present, so the person actually driving has a way to reach the customer.

**A new Super Admin panel to price the matrix.** "🔀 Delivery Fee by Zone Pair," added under the existing Delivery Zones settings panel: two zone pickers (vendor zone, customer zone) plus a fee amount and a "Set Fee" button, backed by real CRUD routes (`/api/super-admin/zone-pair-fees`) mirroring the existing Delivery Zones panel's own pattern exactly (including a public read route, `/api/delivery-zone-pair-fees`, so Checkout's fee preview can be accurate before the customer submits — same reasoning as the existing public `/api/delivery-zones` route). Setting a fee for a pair that already has one updates it in place (`ON CONFLICT ... DO UPDATE`) rather than creating a duplicate, the same re-import-safe pattern already used by the Delivery Zones bulk importer.

**Verified.** `schema.sql` (the new `zone_pair_fees` table, its unique `(vendor_zone_id, customer_zone_id)` constraint and two indexes, `orders.sender_phone`, `purchases.pending_sender_name`/`pending_sender_phone`) applied clean against a real disposable local Postgres database, confirmed idempotent on a second run (`IF NOT EXISTS` skipped everything correctly), and `\d` confirmed the exact expected column/constraint shape. `node --check` passed on `server.js`, `db.js`, and the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline. A Playwright pass against the real, edited `index.html` (API calls mocked) drove the actual Checkout Modal end-to-end: confirmed Name/Phone pre-fill from the mocked logged-in account; confirmed picking a saved address with a zone attached made the fee preview show the real zone-pair price ($12.50 for a seeded Zone A → Zone B pair) instead of the vendor's flat $3 fee; confirmed typing a new address by hand cleared the resolved zone and the preview fell back to the flat $3 fee, exactly as designed; and confirmed the actual submitted checkout request body carried the edited name/phone and the resolved dropoff zone id. A second Playwright pass drove the new Super Admin panel directly: confirmed the two zone dropdowns populate from the real zone list, adding a new pair fee sends the right request and the list re-renders to include it, and deleting a pair removes it from the list. A third pass re-ran the two pre-existing hamburger-button and cart-layout Playwright regression suites from the two fixes above against the fully edited file, confirming zero regressions there. Zero JavaScript exceptions across every pass.

## Checkout: free-typed dropoff addresses removed, a delivery zone is now mandatory

**Requested:** a follow-up to the zone-pair delivery fee feature above — "Remove customers typing a address by hand instead and force zone pick." The feature above still let a customer type a brand-new dropoff address directly into Checkout with no zone attached at all, which silently fell back to the vendor's flat zone fee — the exact loophole that made the new zone-pair pricing optional rather than actually enforced. Two design decisions were confirmed with the user before writing any code: Checkout keeps the ability to add a brand-new address inline (not just pick from existing saved ones), but the zone becomes mandatory for it, same as picking an existing one; and a saved address from before this requirement existed that has no zone on it is simply hidden from the Checkout picker (not deleted, not blocking) until a zone is added to it.

**What actually changed.** The Dropoff Address field's free-text input is now `type="hidden"` — there is no text box left to type into directly. In its place, the dropdown only lists saved addresses that already have a zone attached, plus a "+ Add a new address" option; picking that opens an inline mini-form (Label, Address, and a required Zone Search Picker) that saves a real entry to the customer's own address book — the same `POST /api/addresses` endpoint Settings already uses — rather than a one-off scratch value, so it's there to reuse next time too. If a customer has zero zoned addresses saved, Checkout skips straight to that inline form instead of showing an empty, dead-end dropdown. Submitting is blocked client-side with a clear message until both an address and its zone are resolved, and the two live checkout routes (`/api/marketplace/checkout/multi`, `.../multi/momo-manual`) now reject a request with no `dropoffZoneId` outright (400) — a real server-side backstop, not just a UI nicety, since the frontend enforcement alone could always be bypassed by a direct API call.

**The ripple that had to be closed too.** Since a saved address' zone now drives real checkout pricing, `POST /api/addresses` was changed to require `zoneId` on creation (a saved address can no longer be born zoneless), and the same requirement was added to the two Settings "Saved Addresses" forms (`#address-form` in the Marketplace app, `#dcust-address-form` in the Delivery Customer app) that already used the existing Zone Search Picker widget — it previously labeled the zone field "(optional)" there; without closing this gap, a customer could still create a zoneless address from Settings that would then just mysteriously never show up at Checkout, with no explanation. `zoneSearchPickerHtml()` gained a `required` parameter for exactly this — set for these two forms and the new Checkout inline form, left unchanged (still optional) everywhere else this same shared widget is used (a vendor's or delivery company's Home Base zone, a customer's own preferred zone in Settings), since those weren't part of what was asked here. `PUT /api/addresses/:id` still lets the "Set as Default" quick action update just `isDefault` without touching zone (it never sends the field at all) — the new requirement only fires when `zoneId` is actually part of the request, exactly like the existing "omitted = leave unchanged" convention that route already had.

**Verified.** `node --check` passed on `server.js` and the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline. Three Playwright passes drove the real, edited Checkout Modal end-to-end: one confirmed a zoneless saved address is genuinely excluded from the dropdown (only the zoned one and "+ Add a new address" appeared) while the zoned one auto-selects by default; one confirmed the inline "add a new address" form blocks saving with a clear error when no zone is picked, then succeeds and auto-selects the new address once a zone is chosen via the picker, with the real `POST /api/addresses` request carrying the zone; and one confirmed that with zero saved addresses at all, Checkout opens straight into the inline form instead of a dead-end empty dropdown, and that submitting with nothing filled in is blocked before any request ever reaches the server. Direct source inspection confirmed the server-side `dropoffZoneId` guard is present and correctly placed in both live checkout routes, and that `POST /api/addresses` rejects a missing `zoneId`. The full Playwright regression suite from every prior feature this session was re-run against the fully edited file — zero new failures (the one pre-existing, unrelated desktop-viewport timeout in the hamburger-fix suite was independently reproduced against the last-delivered zip from before this session's changes even began, confirming it's not a regression from any work done here).

## Bulk import for the Delivery Fee by Zone Pair chart

**Requested:** "I need to be able to import Delivery Fee by Zone Pair chart" — setting every pair one at a time through the panel's own Vendor Zone / Customer Zone / Fee form doesn't scale once there are more than a couple of zones (N zones means up to N² pairs), the same reason the pre-existing Delivery Zones panel already has its own "Import Regions & Zones" bulk-paste button.

**The format.** A chart, not a flat list, since a zone-pair fee is inherently two-dimensional — matching how an admin would naturally lay this out in a spreadsheet: a header row of customer zone codes, then one row per vendor zone code followed by that row's fees. Tab-separated (an ordinary paste straight out of Excel or Google Sheets) or comma-separated both work, auto-detected per line by whether it contains a tab. A blank cell is treated as "skip this pair" — left unset, or left however it already was — never coerced into a $0 fee, since blank and zero mean different things for a real delivery charge.

**Parse-everything-first, apply-nothing-if-anything's-wrong.** `parseZonePairFeesImportText()` (server.js) turns the pasted text into `{vendorCode, customerCode, fee}` triples, collecting every row/column error (an unparseable fee, a missing vendor code, a duplicate pair within the same paste) rather than stopping at the first one — so a Super Admin fixing a large chart gets the whole list of what's wrong in one pass, exactly like the existing Delivery Zones importer's own error style. `POST /api/super-admin/zone-pair-fees/import` then resolves every zone code against the real `delivery_zones.code` column *before* writing anything — an unknown code (typo, or a zone that doesn't exist yet) rejects the entire import with the exact unknown codes named, rather than silently skipping just that row and leaving the admin to notice something's missing. Only once every code and every fee in the whole pasted chart checks out does `db.importZonePairFees()` run, as one transaction, upserting each pair with the same `ON CONFLICT (vendor_zone_id, customer_zone_id) DO UPDATE` pattern the single-pair "Set Fee" action already uses — so re-importing a chart later (e.g. after tweaking a few numbers) safely updates existing pairs in place instead of duplicating them, and a mid-import failure can never leave the table half-updated.

**Where it lives.** A new "⬆ Import Zone Pair Fees" button next to "Set Fee" in the existing panel opens a modal with a monospace textarea and an example chart shown above it — same modal shape, same open/close/submit wiring, same error/summary display pattern as the pre-existing Delivery Zones import modal right next to it, down to the DOM-ordering comment explaining why it's placed after the modal it's opened from (so it stacks on top and actually receives clicks).

**Verified.** `node --check` passed on `server.js` and `db.js`. Duplicate-static-id and `<div>`-balance checks held at baseline. `parseZonePairFeesImportText()` was extracted and run directly against five cases: a full valid tab-separated chart with a blank cell (confirmed the blank cell was skipped, not imported as $0, and every other cell parsed with the right vendor/customer code and fee); the same chart comma-separated (identical result); an invalid fee value (correctly rejected with the exact line and column named); a row missing its vendor zone code (rejected, line named); and a chart with only one line (rejected before even attempting to read a data row). The exact SQL `db.importZonePairFees()` runs was hand-verified against a real disposable local Postgres database: importing two new pairs created exactly two rows, and re-importing one of those same pairs under a different generated id updated the existing row's fee in place rather than creating a duplicate (row count stayed at 2, the original row's id was unchanged, its fee reflected the new value) — proving the "safe to re-run" claim by construction, not just by reading the code. A Playwright pass drove the real Import modal end-to-end: opened it via the actual button click, submitted a chart referencing an unknown zone code and confirmed the server's exact error text surfaced in the modal, then submitted a valid chart and confirmed the success summary appeared and the underlying zone-pair fee list visibly refreshed to include the newly imported pairs. The full Playwright regression suite from every prior feature this session was re-run against the fully edited file with zero new failures.

## Fixed: delivery fee showing $0.00 for products opened from Wishlist/a vendor's Store page/"More from this store"/"customers who bought this also bought" — plus vendors can now see the actual items (with photos) in each order

**Reported:** a screen recording showing Checkout's Delivery Fee line stuck at $0.00 despite a real zoned dropoff address being selected, plus a request that vendors be able to open an order and see the item list with photos, not just a buyer name and a total.

**Root cause of the $0.00 fee.** `zonePairFeeFor()` (the function that prices each vendor's line — see the zone-pair-fee feature above) returns $0 immediately whenever the cart line's `vendorDeliveryZoneId` is missing, by design — it's the "this vendor has no zone assigned" case, never meant to silently guess a number. That field rides on the product object from the moment it's added to the cart (`addToCart()` copies `product.vendorDeliveryZoneId` straight onto the cart line), so it's only ever as good as whatever query fetched that product. `getActiveProductsForStorefront()` (the Home tab feed) has always joined `users.delivery_zone_id` in correctly — but four other product-listing queries that feed the exact same product cards and the exact same `addToCart()` call never did: `getWishlist()` (the Wishlist tab), `getVendorStorefrontProducts()` (opening a vendor's own Store page directly), `getRelatedVendorProducts()` ("More from this store" on a product page), and `getCoPurchasedProducts()` ("Customers who bought this also bought"). A product added to cart through any of those four paths carried `vendorDeliveryZoneId: undefined` no matter how correctly a Super Admin had configured that vendor's zone or the zone-pair fee table — which reads as "the delivery fee feature is broken" from the outside, when the fee logic itself was actually working correctly the whole time; the recording's product just happened to have been opened through one of the four unjoined paths. The same four queries were also missing `vendor_store_address`, so the same products would have shown a blank/wrong Pickup Address at Checkout too. All four now join `u.store_address AS vendor_store_address, u.delivery_zone_id AS vendor_delivery_zone_id` exactly like the Home feed always has, so every path a customer can reach a product from now carries the same fee-relevant fields onto the cart line.

**Vendor order visibility.** `getPurchasesByVendor()` (backing the vendor's own Orders tab) returned the purchase total and buyer name only — no line items at all — while the customer's own Order History (`getPurchasesByCustomer()`) already had a real `items` subquery (product name, quantity, current image, color/size if applicable) that just was never given to the vendor side. `getPurchasesByVendor()` now runs the identical subquery, and each order row in the vendor's Orders tab renders the same item-thumbnail strip (image, name, "Qty N", plus color/size when the product has variants) the customer's purchase cards already use — reusing the existing `.mp-purchase-items-scroll`/`.mp-purchase-item` styling rather than inventing a new look, so a vendor can now see exactly what was ordered, photos included, directly in the Orders list without opening anything separate.

**Verified.** `node --check` passed on `db.js`. Duplicate-static-id and `<div>`-balance checks held at baseline. Every edited query was hand-run against a real disposable local Postgres database with seeded vendor/zone/product/wishlist/purchase rows — confirmed `getWishlist`'s query now returns `vendor_delivery_zone_id`/`vendor_store_address`, confirmed the same for `getVendorStorefrontProducts`/`getRelatedVendorProducts`/`getCoPurchasedProducts`, and confirmed `getPurchasesByVendor`'s query returns a correctly-shaped `items` array. A Playwright pass against the real, edited `index.html` (API calls mocked) reproduced the bug end-to-end first — a product shaped exactly like the four previously-broken queries now return (i.e. carrying `vendorDeliveryZoneId`) was added to cart, and the delivery fee preview correctly showed the vendor's flat zone fee before a customer zone was chosen and the real, higher zone-pair fee once one was — then confirmed the vendor Orders tab renders a real item strip (two items, one with a variant, correct images/names/quantities) instead of the previous bare row. Zero JavaScript exceptions across the pass.

## Vendor Dashboard now shows only pending orders; Order History moved to the Orders tab — plus a delivery zone is now required when Super Admin adds a vendor directly

**Requested:** "keep only pending orders on venders dashboard and the other orders at the orders section" — the Dashboard's "Recent Orders" panel and the Orders tab ("All Orders") previously showed the same unfiltered purchase list (the Dashboard just capped at 3), so a vendor had no way to see only what still needed their attention without the fulfilled/cancelled history mixed in. Second, separately: "at Super Admin + Add Vendor section, please add venders zones as a requirement (force to enter)" — the delivery-fee-by-zone feature (and the $0.00 fee bug fixed above) both depend on a vendor having a zone assigned, but the Super Admin's own "+ Add Vendor" form never asked for one at all, so a vendor created this way started zoneless with nothing surfacing that gap until a customer's delivery fee mysteriously priced at $0.

**The Dashboard/Orders split.** Purchases now split by the same "is this resolved yet" logic `vendorOrderRowHtml`'s status pill already used (`delivered`/`cancelled` = resolved, everything else — `placed`/`pending`/`accepted`/`picked-up` — still open) via a new `isVendorOrderPending()` helper, and the split is mutually exclusive: the Dashboard's panel (retitled "Pending Orders," its "View All ›" link retitled "Order History ›") shows every still-open order — no longer capped at 3, since hiding an actionable order past the third row would defeat the point — and the Orders tab (retitled "Order History") shows only Fulfilled/Cancelled orders. A vendor never has to check both places for the same order. All of a row's existing behavior (dispatch/cancel/dismiss/deletion-request buttons, the item-photo strip added above) is unchanged — only which of the two containers a given row renders into moved.

**The required zone on Add Vendor.** A new required Zone Search Picker (the same reusable widget used everywhere else a zone is picked in this app) sits in the "+ Add Vendor" modal right after Business Type, wired through `initZoneSearchPickers()`'s existing table like every other instance. Submitting with no zone picked is blocked client-side with the same "Search for and select a delivery zone" message the Checkout/Saved-Addresses required pickers already use; `POST /api/super-admin/vendors` independently rejects a missing `deliveryZoneId` server-side too (never trust the client alone, same posture as every other required-field enforcement this session), and validates the zone id actually exists before creating the account. `db.createUser()` already accepted a `deliveryZoneId` parameter (added for self-registration) — this just wires the Super-Admin-direct path to actually pass it through, so a vendor created this way is correctly zoned from account creation, not left to be discovered broken later at checkout.

**Verified.** `node --check` passed on `server.js` and the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline. `db.createUser`'s exact INSERT was hand-run against a real disposable local Postgres database with a `deliveryZoneId` supplied — confirmed the new vendor row's `delivery_zone_id` was set correctly. A Playwright pass against the real, edited `index.html` (API calls mocked) drove `renderVendorOrders()` with six purchases spanning every status — confirmed the four still-open statuses (placed/pending/accepted/picked-up) rendered into the Dashboard's Pending Orders panel and only the two resolved statuses (delivered/cancelled) rendered into the Orders tab, with zero overlap. The same pass drove the real Add Vendor modal end-to-end: confirmed the zone picker renders, confirmed submitting with no zone selected is blocked client-side with no request sent to the server, and confirmed picking a real zone and resubmitting sends the correct `deliveryZoneId` in the POST body. The existing zone-pair-fee, forced-zone-checkout, and stock-enforcement Playwright regression suites were re-run against the fully edited file with zero new failures — confirming the shared `initZoneSearchPickers()`/`zoneSearchPickerHtml()` widget still works correctly everywhere else it's used after adding this fourth required instance.

## Fixed: a voided order stayed stuck in Pending Orders and still offered "Send to Delivery"

**Reported:** a screen recording of the Vendor Dashboard's new Pending Orders panel (from the split above) showing an order marked "Voided (not counted)" sitting right next to a "📦 Send to Delivery" button — the vendor's actual request was "I need only pending orders to show here" plus "when an order gets Voided (not counted), please remove the Send to Delivery button."

**Root cause.** The Pending vs. Order History split only looked at the order's *delivery* status (`delivered`/`cancelled` vs. everything else) — it never considered `excludedFromRevenue` (the "Voided" flag a Super Admin sets when resolving a dispute; see the schema.sql comment on that column). Voiding doesn't touch the underlying delivery status at all, so a voided order whose delivery was never marked Delivered or Cancelled — the exact case in the recording, still sitting at "Processing" — kept showing as pending indefinitely, and its dispatch button had the same blind spot: `canDispatch` in `vendorOrderRowHtml` only checked `status === 'pending'`, with no awareness that the order had already been closed out from underneath it.

**The fix.** `isVendorOrderPending()` now also requires `!p.excludedFromRevenue` — a voided order is treated as resolved regardless of its delivery status, so it moves into Order History alongside genuinely fulfilled/cancelled ones instead of lingering in Pending Orders forever. `canDispatch` gained the same `!p.excludedFromRevenue` condition, so "Send to Delivery" never renders next to a "Voided (not counted)" badge — dispatching an order for real pickup/delivery makes no sense once a Super Admin has already decided it doesn't count.

**Verified.** `node --check` passed on the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline. A Playwright pass against the real, edited `index.html` reproduced the exact recording scenario — three purchases: a genuinely-open "Placed" order, a genuinely-open "Processing" order, and a third "Processing" order that's also voided — and confirmed only the two genuinely-open orders landed in the Pending Orders panel, the voided one landed in Order History instead, the voided row's "Send to Delivery" button was absent while its "Voided (not counted)" badge was present, and the non-voided "Processing" order's dispatch button was untouched (still shows, confirming this didn't over-hide the button for legitimate orders). The prior Dashboard/Orders-split, Add-Vendor, forced-zone-checkout, and zone-pair-fee Playwright suites were re-run with zero new failures.

## Fixed: cancelled Mobile Money orders still showed "Processing" in the customer's own Order History

**Reported:** "check customer side, all cancelled orders still showing Processing" — a follow-up to the vendor-side voided-order fix above, this time on the customer's own "Your Orders" screen.

**Root cause.** `renderMarketplacePurchases()`'s status pill fell back to "Processing" for *any* purchase with no `deliveryStatus` yet — a reasonable-looking rule, but it silently assumed "no delivery status yet" always means "still being worked on." That assumption breaks for Mobile Money: `checkout()` deliberately defers creating a real delivery order until the payment is confirmed (see that function's own comment), so a `momo`/`momo_manual` purchase whose payment was rejected or cancelled *before* ever being confirmed — via the customer's own Cancel action, a vendor's approved cancellation request, or a Super Admin rejecting the payment, all three of which funnel through the same `db.voidFailedMomoPayment()` — never gets a delivery order at all. `deliveryStatus` stays null for that purchase permanently, not just "for now," so the fallback kept showing "Processing" forever for an order that had actually died. The payment badge right below it already said "Payment Failed"/"Payment Not Confirmed" correctly — the two pieces of the same card were contradicting each other.

**The fix.** The status pill now checks `paymentStatus === 'failed'` (only ever true for a dead `momo`/`momo_manual` payment — `cod` purchases default to `not_applicable` and never reach `'failed'`) whenever `deliveryStatus` is null, and shows "Cancelled" with the same styling a genuinely-cancelled delivery order already gets, instead of "Processing." A still-pending Mobile Money payment (not yet rejected or confirmed) is unaffected and still correctly shows "Processing." While in this code, the same dead-payment case was also quietly making "Request a Return" available on an order that was never created or fulfilled in the first place (its condition only checked "no delivery order exists yet," which a dead payment also satisfies) — that button is now correctly hidden for it too, alongside the "Track Delivery" button (already correctly absent, since there's no delivery order to track).

**Verified.** `node --check` passed on the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline. A Playwright pass against the real, edited `index.html` drove `renderMarketplacePurchases()` with four purchases: an active, not-yet-dispatched COD order (confirmed it still correctly shows "Processing," proving the fallback wasn't over-corrected), a genuinely-cancelled delivery order (confirmed unaffected — already worked, still "Cancelled"), a rejected/dead Mobile Money payment (confirmed it now shows "Cancelled" instead of "Processing," and confirmed no "Request a Return" button appears on it), and a still-pending Mobile Money payment (confirmed it correctly still shows "Processing," not prematurely "Cancelled"). The vendor-side voided-order, purchase-dispute, and payment-badge Playwright regression suites were re-run against the fully edited file with zero new failures.

## Follow-up fix: the vendor's own status pill still said "Processing" (or, inconsistently, "Cancelled") for a voided order

**Reported:** a screenshot of the vendor's Order History tab showing two voided orders side by side with two *different* top-right status pills — one said "Processing," the other said "Cancelled" — both sitting right next to the same "Voided (not counted)" badge, with the reply "Something still need fixing."

**Root cause.** The vendor-side voided-order fix earlier in this document only touched two things: which container a voided row renders into (`isVendorOrderPending`) and whether its "Send to Delivery" button shows (`canDispatch`). It never touched the status pill's own `statusLabel`/`statusClass` computation in `vendorOrderRowHtml`, which still derived purely from the order's raw delivery status — `pending`/`accepted`/`picked-up` → "Processing", `cancelled` → "Cancelled", with no awareness of `excludedFromRevenue` at all. Since voiding doesn't touch delivery status, two voided orders that happened to be voided at different points in their lifecycle (one before ever being marked cancelled, one after) ended up showing two different, contradictory-looking pills — exactly what the screenshot caught.

**The fix.** `excludedFromRevenue` now overrides the pill outright: any voided order shows a single, consistent "Voided" label (reusing the existing red `cancelled` pill style, since both mean "this order isn't going anywhere"), regardless of what its underlying delivery status happens to be. A non-voided order's pill is completely unaffected.

**Verified.** `node --check` passed on the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline. A Playwright pass against the real, edited `index.html` reproduced the exact screenshot — two voided orders, one with delivery status `pending` and one with delivery status `cancelled` — and confirmed both now render the identical "Voided" pill, plus a third, non-voided control order confirming its "Processing" pill is untouched. The vendor Dashboard/Orders-split, Add-Vendor, and customer-side cancelled-status Playwright suites were re-run against the fully edited file with zero new failures.

## Same voided-order pill inconsistency, checked and fixed on the customer side too

**Requested:** "It was fixed. Please check customers side too" — after confirming the vendor-side "Voided" pill fix, a direct follow-up asking whether the customer's own Order History has the same gap.

**It did.** `renderMarketplacePurchases()` (the customer's "Your Orders" cards) had no `excludedFromRevenue` handling anywhere — unlike the vendor side, which at least had a "Voided (not counted)" badge even before this fix, the customer-facing card had zero indication a purchase had been voided at all. Its status pill fell straight through to whatever the raw delivery status (or the dead-Mobile-Money-payment fallback added in the fix above) computed, meaning a voided order's pill could show "Processing," "Fulfilled," or "Cancelled" depending purely on when in its lifecycle it happened to be voided — the exact same inconsistency just fixed for vendors, just never surfaced to a customer in the screenshots so far since none of the test orders shown had actually been voided yet.

**The fix.** Same rule as the vendor side: `purchase.excludedFromRevenue` now overrides everything else and always renders a single, consistent "Voided" pill (same red `status-cancelled` styling already used for a genuinely-cancelled order), regardless of delivery status or whether it would otherwise have hit the dead-payment "Cancelled" fallback. A non-voided purchase's pill — including the dead-Mobile-Money-payment case fixed earlier — is completely unaffected.

**Verified.** `node --check` passed on the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline. A Playwright pass against the real, edited `index.html` drove `renderMarketplacePurchases()` with five purchases: a voided order that was still "Processing" underneath, a voided order that had already been delivered, a voided order that was also a dead Mobile Money payment (confirming "Voided" wins over the dead-payment "Cancelled" fallback too), and two non-voided controls (an active order and a dead Mobile Money payment) confirming neither was affected. All five rendered exactly as expected. The vendor-side voided-pill and customer-side cancelled-status Playwright suites were re-run against the fully edited file with zero new failures.

## Checked the Super Admin side for the same bug classes — found and fixed a real one: voided sales were still counted in the Platform Report's revenue

**Requested:** "Can you check Admins side too to see if any of these fixed needed there?" — after fixing the voided-order pill inconsistency on both the vendor and customer sides, a direct ask to check whether Super Admin's own screens have the same gaps.

**Most of Super Admin's purchase/order-facing screens turned out to be unaffected, each for a specific, checkable reason.** The Admin "Orders" section renders the `orders` table directly — real, ground-truth `order.status`, not a purchase's derived status — and `excludedFromRevenue` doesn't exist on that table at all, so there's no "which fallback wins" ambiguity to get wrong there. The Disputes panel shows the *dispute's* own status (open/resolved/rejected), a completely separate field from a purchase's delivery/payment status, and its `viewDisputeModal()` already correctly special-cases a voided resolution ("Voided — excluded from revenue"). The Mobile Money Payments queue only ever lists `paymentStatus === 'pending'` rows by construction, so it never renders a "Processing"/"Cancelled"/"Voided" derived label in the first place. And a repo-wide search confirmed `vendorOrderRowHtml`/`renderVendorOrders` (vendor) and `renderMarketplacePurchases` (customer) are the only two places that ever built the "Processing/Cancelled/Voided" pill pattern this session kept fixing — there's no third, Admin-facing copy of it anywhere.

**What Super Admin *did* have wrong: the Platform Report PDF.** Every revenue figure elsewhere in this app that touches `purchases` explicitly excludes voided ones server-side — `db.js` has `AND NOT excluded_from_revenue` (or `AND NOT p.excluded_from_revenue`) baked into the vendor sales-overview query, the vendor customer-totals query, and every Super Admin overview/dashboard stats query. `generatePlatformReportPDF()` — the client-side function behind Super Admin's "📊 Platform Report" button (Monthly/Weekly, Delivery + Marketplace combined) — was the one place that didn't follow that rule: it summed `monthPurchases.reduce((sum, p) => sum + (p.totalAmount || 0), 0)` straight from `db.getAllPurchases()`'s raw result with no `excludedFromRevenue` filter at all. A voided/refunded sale therefore still inflated "Marketplace Revenue," the daily marketplace breakdown, the "Grand Total Revenue" figure, and the "Top Vendors" ranking (and could make a vendor whose only purchase was voided still show up in that ranking) — the official platform-wide report a Super Admin would hand to stakeholders was silently wrong in exactly the way `excluded_from_revenue`'s name says it shouldn't be. The same gap existed one level down in the vendor's own Monthly Report PDF (`generateVendorMonthlyReportPDF()`) — a vendor's self-generated report also summed raw `totalAmount` with no voided-purchase exclusion.

**The fix.** Both functions now filter `monthPurchases` to drop `excludedFromRevenue` purchases immediately after the date-range filter, before any revenue/count/breakdown math runs — matching the exact convention already used everywhere else in the codebase (voided purchases are excluded from *both* the dollar totals and the purchase counts, not just the dollars, same as the SQL `NOT excluded_from_revenue` sites do). Every downstream number in both PDFs (Marketplace Revenue, Marketplace Purchases count, the daily breakdown, the Top Vendors table, Total Sales, Total Purchases) now automatically reflects the exclusion, since they all derive from that one filtered array.

**Verified.** `node --check` passed on the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline. Two Playwright passes drove the real, edited `index.html`, mocking `window.jspdf` to capture every `doc.text()` call the real report-generation code produces (rather than parsing a rendered PDF binary) — no reliance on the CDN-hosted jsPDF library being reachable. The Platform Report pass used four purchases across three vendors — Vendor A with one real $100 sale and one voided $60 sale, Vendor B with one real $40 sale, and Vendor C whose only $999 sale was voided — and confirmed "Marketplace Revenue" read exactly $140.00 (not $199.00 or $140.00+$999.00), "Marketplace Purchases" counted 2 (not 4), Vendor A's Top Vendors row read exactly $100.00 (not $160.00), and Vendor C — fully voided — was completely absent from the Top Vendors table. The vendor Monthly Report pass used one real $30 sale and one voided $20 sale and confirmed "Total Purchases" read 1, "Total Sales" read $30.00, and the voided purchase's customer name never appeared in the report's line-item table. The accumulated Playwright regression suite from every prior fix this session (delivery-fee-join, vendor Dashboard/Orders split, voided-order pending/dispatch, customer cancelled-status, vendor voided-pill, customer voided-pill) was re-run against the fully edited file with zero new failures — one older, pre-existing test script (written before the Dashboard/Orders split existed) queried a purchase-item selector against the wrong container id for a since-added "pending orders render into the preview container" behavior; confirmed by direct inspection that the item itself still rendered correctly, just into the container the later split moved it to — not a regression from this fix, which touches only the two PDF-report functions.

## Follow-up: the rest of the Admin-side upgrade list — fixed all four

**Requested:** "Please check and see if anything else need to be upgraded on Admins side and let give me the list" — a broader sweep than the specific bug classes already checked, followed by "Start!" once the list came back. The audit (three parallel investigations covering Orders/Disputes/Payments/Overview, Vendors/Zones/Banners/Staff, and a TODO/README sweep) turned up four real, concrete issues, all now fixed.

**1 & 2 — the Platform Report and vendor Monthly Report PDFs still summed unrealized Mobile Money revenue, and "Completed Payments" always read 0.** The `excludedFromRevenue` fix from the prior entry only closed one of two gaps these two functions had relative to every SQL revenue query in `db.js`. Every real revenue query (`getMarketplacePlatformStats`, `getBusinessOverviewStats`, `getPayoutSummary`, vendor sales overview) requires `(payment_method = 'cod' OR payment_status = 'successful')` before counting a dollar — a still-pending or rejected Mobile Money purchase isn't revenue yet. Both PDF functions skipped that condition entirely, so a customer's Mobile Money purchase that was never actually paid for still inflated Marketplace Revenue, the daily breakdown, Grand Total Revenue, and the Top Vendors ranking. Separately, "Completed Payments" was checking `paymentStatus === 'completed' || paymentStatus === 'paid'` — values that don't exist anywhere in this app's actual vocabulary (`not_applicable` / `pending` / `successful` / `failed`, per `schema.sql`'s own CHECK constraint) — so that line had been silently reading 0 in every report ever generated, regardless of real sales volume. Both functions now derive a `realizedPurchases` subset (`paymentMethod === 'cod' || paymentStatus === 'successful'`) and use it for every dollar figure and the completed-payments count, while the raw purchase-attempt count and (for the vendor report) the itemized per-purchase table still show every attempt, pending and failed ones included, so nothing is hidden — only the revenue math changed.

**3 — a vendor's delivery zone could be silently cleared after creation, with zero warning, quietly reopening the exact bug the required-zone-on-creation fix closed.** The Vendors table's own zone dropdown lets a Super Admin pick "— No zone —" on any existing vendor in one click, and the backend endpoint accepted `zoneId: null` unconditionally. Clearing a zone this way is a legitimate, intentional capability (e.g. mid-reassignment) — not something to remove — so instead of blocking it, picking "— No zone —" on a vendor that currently has one now shows the same kind of explicit confirmation the Delivery Zones panel already gives before a zone deletion with the same effect: "Clear \<vendor\>'s delivery zone? Every customer's delivery fee for this vendor will show as $0.00 until a zone is assigned again." Cancelling leaves the vendor's zone untouched and reloads the table to show the real, unchanged value; confirming proceeds as before. Assigning a *real* zone (including to a vendor that had none) is unaffected — no dialog, since that's never the risky direction.

**4 — that same endpoint accepted an unvalidated zone ID.** Every other zone-accepting route in the app (`/api/auth/register`, `/api/super-admin/vendors` at creation, `/api/me/profile`, saved-address routes) runs a real zone ID through the shared `validateOptionalZoneId()` check before writing it; `PUT /api/super-admin/vendors/:id/delivery-zone` was the one route that skipped it and let a bad ID fall through to the database's foreign key as an opaque `500`. It now runs the same shared check — `null`/omitted (clearing) is still allowed, but a non-empty zone ID that doesn't exist now returns a clean `400 Selected delivery zone was not found` instead.

**5 — the Disputes list couldn't show "Voided" at a glance.** `viewDisputeModal()` already correctly labeled a voided-purchase resolution as "Voided — excluded from revenue" once an admin opened that specific dispute, but the main Disputes table just showed the same generic green "Resolved" pill for a void, a refund, or a plain no-action resolution alike — there was no way to tell them apart while scanning the list. The list row now applies the identical `excludedFromRevenue`-on-a-resolved-dispute check the modal already used, showing a distinct "Voided" pill inline; a genuinely-resolved (non-voided), still-open, or rejected dispute's pill is completely unaffected.

**Verified.** `node --check` passed on `server.js` and the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline. Four new Playwright passes drove the real, edited `index.html`: one confirmed the Platform Report's realized-revenue math directly (four purchases — COD, successful Mobile Money, pending Mobile Money, and failed Mobile Money — confirmed the pending and failed ones were excluded from both revenue and Top Vendors while still counting toward the raw purchase-attempt total, and confirmed "Completed Payments" correctly read 2, not 0); a matching pass confirmed the same for the vendor's own Monthly Report; one drove the real Vendors table end-to-end — clearing an existing zone and cancelling the confirm sent no request, clearing and accepting sent `{zoneId: null}`, and assigning a zone to a previously-zoneless vendor sent the request immediately with no dialog at all; and one drove the real Disputes list with four disputes (a refunded resolution, a voided resolution, an open dispute, and a rejected dispute) and confirmed only the voided one showed "Voided," with the other three pills unaffected. The accumulated Playwright regression suite from every fix this session was re-run — two of the session's own earlier tests needed their fixture data corrected (they used a `paymentStatus: 'completed'` value that doesn't exist in this app's real vocabulary, which the new realized-payment filter correctly treats as unrealized; fixing the fixture to a real `paymentMethod: 'cod'` purchase, matching what those tests always intended to represent, confirmed the expected numbers hold) — not an application regression, since no real purchase can ever actually carry that payment_status value. Every other test in the suite passed unchanged.

## Follow-up: the customer-side upgrade list — fixed all five

**Requested:** "Please check and see if anything else need to be upgraded on customers side and let give me the list," followed by "Start!" once the list came back. The audit (three parallel investigations covering the shopping flow, the order/dispute/address lifecycle, and Settings/Payment) turned up five real, concrete issues, all now fixed.

**1 — the customer's own "My Reports" dispute list had the exact same "Voided" gap the Admin list just had.** `loadMyDisputesPanel()` rendered the raw dispute-status pill with no override for a voided resolution — the vendor's list, the Admin's list, and the dispute's own detail popup all correctly showed "Voided," but a customer checking their own reported problems still saw a plain "Resolved." Same data (`purchase.excludedFromRevenue`) was already coming back from `/api/disputes/mine`; this was purely a missed spot in the render function. Fixed with the identical override pattern used for the Admin list.

**2 — "Request a Return" could be submitted on a purchase that had already been voided.** Same `excludedFromRevenue` gap already closed twice on the vendor side (the dispatch button, the deletion-request button) but never applied to the customer's own return button — or to the server route behind it. `POST /api/returns` now rejects a voided purchase with a real 400 before even checking delivery status, and the button itself no longer renders for one, matching the "client hides it, server enforces it" pattern used everywhere else this kind of guard exists in this app.

**3 — a product a Super Admin hides for moderation could still be bought if a customer had already wishlisted it or had it sitting in their cart.** Every browse query in the app (`getActiveProductsForStorefront` and its related/co-purchased/vendor-storefront siblings) filters `is_active = true`, but `getWishlist()` never did, and — more importantly — `checkout()`'s own per-item validation loop never checked a product's active flag at all, even though it re-validates price, stock, and color/size fresh from the database on every single checkout. So a Super Admin's "Hide" action (meant to pull a bad listing from sale) could be silently bypassed by anyone who'd already wishlisted or cart-added the product beforehand. `getWishlist()` now excludes hidden products the same way every other listing query does, and — the fix that actually matters — `checkout()` now rejects any item whose product is `is_active = false` with a clear "\<product\> is no longer available" error, closing the gap at the one place that actually creates a sale, not just at the browse layer.

**4 — the Account → Payment Methods panel told customers Mobile Money didn't exist, right after they could have just used it.** The panel was a static "Coming Soon — there's no online payment gateway yet, every order is pay on delivery" message written before Mobile Money checkout (MTN + Orange, manual reference-code flow) shipped, and never updated afterward. It's now rendered dynamically from the same live provider list (`publicPlatformConfig.momoProviders`) Checkout itself already uses — showing Pay on Delivery plus whichever Mobile Money providers are currently enabled by name, and correctly falling back to the old "pay on delivery only" message in the one case where that's still actually true (a Super Admin has disabled every provider). The account-menu row's static subtitle ("Coming soon — pay on delivery for now") was also swapped for neutral copy that stays accurate regardless of provider configuration, rather than trying to keep two separate copies of the same dynamic fact in sync.

**5 — "Buy Now" silently checked out a customer's entire cart, not just the product they clicked.** `Buy Now` was never a separate single-item purchase path — it added the product to the same shared cart Checkout always operates on, then opened Checkout on that whole cart. If a customer already had unrelated items sitting in their cart from earlier browsing, clicking Buy Now on something new bundled everything into one combined order with no indication that wasn't a single-item purchase. Rather than rebuilding cart/checkout state management to isolate a single item (a much larger change for what's fundamentally a "surprise, not a crash" problem), Buy Now now shows a plain confirmation — "Buy Now will check out this item together with the N other item(s) already in your cart, as one order. Continue?" — whenever the cart already holds something else, using the same `confirm()` pattern already established elsewhere in this app for consequential-but-legitimate actions (e.g. clearing a vendor's delivery zone). Buy Now on an otherwise-empty cart, or adding more of a product already in the cart, is completely unaffected — no dialog, since neither is a surprise.

**Verified.** `node --check` passed on `server.js` and the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline. Five new Playwright passes drove the real, edited `index.html`: one confirmed the customer disputes list shows "Voided" only for the voided-resolution dispute, leaving a genuine resolution, an open dispute, and (in the Admin-list version from the prior entry) a rejection all unaffected; one confirmed Request a Return is absent from a voided+delivered purchase card while still present on a non-voided control; one confirmed the Payment Methods panel renders the real, current provider list (including falling back correctly when none are enabled) instead of static copy; and one drove the real Buy Now button end-to-end — cancelling the confirm left the cart completely untouched, accepting it added the new item alongside the existing ones, and Buy Now on an empty cart produced no dialog at all. The `is_active`/moderation-bypass fix was verified by hand-running the exact `getWishlist()` SQL against a real disposable local Postgres database seeded with one hidden and one active wishlisted product — confirmed only the active one returned — and by confirming `checkout()`'s fresh per-transaction product read correctly reports `is_active = false` for the hidden product, the same data the new guard clause checks. The accumulated Playwright regression suite from every fix this session — Admin-side and customer-side both — was re-run against the fully edited file with zero new failures.

## Follow-up: the vendor-side upgrade list — fixed all ten

**Requested:** "Please check and see if anything else need to be upgraded on Venders side and let give me the list," followed by a request to fix all six main findings plus all four smaller, lower-priority ones flagged alongside them. The audit turned up six concrete issues plus four smaller gaps; every one of the ten is now fixed.

**1 — the vendor's own Reports "Order Status Breakdown" donut didn't exclude voided purchases.** `getVendorOrderStatusBreakdown()` grouped purely by the underlying order status (`delivered`, `cancelled`, etc.), so a purchase a Super Admin had voided in a dispute still counted toward whatever status it happened to be at the moment it was voided — disagreeing with the Orders tab, which already shows "Voided" for the exact same purchase. The query now buckets a voided purchase under its own `'voided'` slice regardless of the underlying delivery status (`CASE WHEN excluded_from_revenue THEN 'voided' ELSE ...`), with a matching gray "Voided" entry added to the donut's color/label maps — same "voided overrides everything" rule already applied everywhere else this bug class has shown up.

**2 — the vendor's own Disputes list was missing the "Voided" pill — the third copy of a bug already fixed twice this session.** `renderVendorDisputesList()` rendered the raw dispute-status pill with no override, so a vendor checking disputes filed against their own purchases saw a plain "Resolved" for one Super Admin had actually voided — the same gap already closed for the Admin list and the customer's own list. Fixed with the identical override pattern (a gray pill, "Voided" text) used in both of those.

**3 — editing a product had no name/price validation; creating one did.** `POST /api/vendor/products` has always rejected a blank name or an invalid/negative price; `PUT /api/vendor/products/:id` never did, so a malformed direct API call (bypassing the form, which always sends both together) could blank out a listing's name or set a nonsense price with no error. The same guard is now applied to the edit route too, but only when `name`/`price` are actually present in that request — preserving the route's existing "omitted field = leave unchanged" partial-update behavior, so a vendor updating only stock or colors/sizes still isn't forced to resend name and price.

**4 — a vendor could silently clear their own delivery zone in Settings with no warning.** The server already validated a non-null zone exists (`validateOptionalZoneId`), but nothing warned before submitting a cleared zone — reopening the exact "$0.00 delivery fee" gap already fixed once for Super Admin clearing a vendor's zone from the Vendors panel. The vendor's own Settings form now shows the same warning ("Every customer's delivery fee for your store will show as $0.00 until a zone is assigned again") before submitting a cleared zone, restoring the previous selection if cancelled.

**5 — the Dashboard "Customers" stat used the wrong, capped dataset.** It was computed as `new Set(purchases.map(p => p.customerId)).size` off the same 50-purchase, most-recent-first list Recent Orders already uses — correct for a low-volume store, silently wrong (under-counting) for any vendor with more than 50 purchases or repeat buyers sitting past that cap. It now reads from `/api/vendor/customers` — the same uncapped, one-row-per-distinct-customer dataset the Customers tab itself already uses correctly — instead of re-deriving a customer count from a list that was never meant to represent the full customer set.

**6 — a pending Mobile Money Premium-upgrade payment vanished on reload, with no way to find, check, or cancel it.** The "check your phone" waiting page (with its status polling and Cancel button) only ever appeared right after initiating a charge — if a vendor reloaded or navigated away before it resolved, the in-progress charge became invisible: not shown as pending, not cancelable, not even checkable, short of contacting support. `loadVendorPremiumPanel()` now checks the same `charges` list the subscription endpoint already returns for a charge still `pending`/`momo`, and if one exists, resumes the exact same waiting page and polling used right after initiation — the first poll immediately reflects whatever actually happened while the vendor was away.

**7 — no self-service way to temporarily hide a listing; only permanent Delete or Super Admin's own toggle.** A vendor wanting to pause a listing (out of stock at a supplier, say) had to either delete it outright or ask a Super Admin to flip their own moderation toggle. The Products tab now has its own Hide/Unhide button per product — same `is_active` field and confirm-before-hiding pattern Super Admin's own product-moderation toggle already uses, just scoped to the vendor's own `PUT /api/vendor/products/:id`.

**8 — coupons didn't check that a start date was actually before its end date.** `validateCouponFields()` already rejected an end date in the past but never compared it against a supplied start date, so a coupon could be created with a start date on or after its own end date — valid at creation, never once redeemable, with no error explaining why. It now rejects that combination outright ("Start date must be before end date") whenever both are supplied.

**9 — no direct link from the Products tab to a vendor's own live listing, so a customer's Q&A question was easy to miss.** Product Q&A worked end-to-end, but the only way to see it was separately browsing to your own storefront as a shopper would. Every active product card now has a "🔗 View Listing" button that opens the vendor's real storefront feed and jumps straight to that product's live page — Q&A section included — the same underlying flow "preview your storefront" already used, just one click away instead of a separate trip. Scoped to active listings only, since a hidden product has no live page to view.

**10 — no in-dashboard way to see reviews on a vendor's own products.** Reviews were only reachable the same way — previewing your own storefront as a shopper would. The vendor dashboard now has its own Reviews tab (desktop sidebar item + a Reviews quick-access link in the mobile Account tab), reading the same public `GET /api/marketplace/vendors/:id/reviews` the storefront page already uses for the vendor's own id. Deliberately not a Premium perk, unlike Customers/Leads/Messages — a review is public storefront content a vendor could already see by browsing their own store, so gating it behind an upgrade would only be adding friction, not access.

**Verified.** `node --check` passed on `server.js`, `db.js`, and the extracted client script. Duplicate-static-id and `<div>`-balance checks held at baseline. The `getVendorOrderStatusBreakdown` rewrite was hand-run against a real disposable local Postgres database seeded with a delivered, a voided-but-formerly-cancelled, and a no-order-yet purchase — confirmed the voided one buckets as `'voided'`, not `'cancelled'` — and the same database confirmed `/api/vendor/customers`'s distinct-customer-count query returns the correct count regardless of purchase volume, the exact number the Dashboard stat fix now relies on. Two new Playwright passes drove the real, edited `index.html` end to end: one covering fixes #2, #5, #7, #9, and #4 together (voided pill in the vendor's own disputes list; the Dashboard Customers stat reading 2 real customers instead of an inflated/duplicated count off a mocked 5-purchase capped list; the Hide button rendering only "Hide"/"Unhide" correctly per product with its confirm dialog firing the right warning text; View Listing appearing only on the active product and correctly opening the storefront-then-PDP overlay stack to the right product; and the zone-clear confirm firing with the right warning text on Settings submit) and one covering fixes #10 and #1 (the new Reviews tab loading and rendering a real average/count and both reviews' author, rating, and comment text; the reports donut's color/label maps confirmed to include the new `voided` entries). Zero page errors across both. The accumulated Playwright regression suite from every fix this session — Admin-side, customer-side, and vendor-side all — was re-run against the fully edited file: the handful of failures are all in fleet/agent/checkout/stock-enforcement/sidebar scripts untouched by this batch, sharing a "DOM element not found" signature consistent with pre-existing staleness rather than anything introduced here; every test touching code this batch actually changed (zone-clear confirm, product edit, voided-pill consistency, the orders split, the edit button) passed cleanly.

## Follow-up: a voided order stayed stuck in "Pending Assignment" forever

**Reported:** the Admin Daily Report kept showing a "Pending Assignment: 1" / "Unassigned Orders (1)" order that the user had already cancelled — `ORD-MT2QZ97GM`, a marketplace order still showing status **PENDING** in both the dashboard stat and the report, with no delivery agent ever assigned.

**Root cause.** Voiding a purchase (Super Admin approving a vendor's "Request Deletion," or resolving any dispute with the 'void' decision) deliberately never touches the linked delivery order — `db.excludePurchaseFromRevenue` only ever updated `purchases`, on purpose, with a comment explaining why: "a delivery company already did the delivery work and keeps that fee no matter why the vendor's own product revenue is later voided." That reasoning is correct once an order has actually been accepted, picked up, or delivered — there's real work to protect. But it was applied unconditionally, including to an order that was **still `pending` and never accepted by anyone**. For that case there's no delivery work to protect at all, so the order just sat there forever: invisible everywhere the voided purchase itself correctly shows as "Voided," but still counted as needing a delivery agent in `calculateAndRenderStats()`'s `todayPending` stat (the dashboard's "Pending Assignment" card) and in every future daily report's "Unassigned Orders" section, since neither of those derive from the purchase at all — they only ever look at raw `orders.status`.

**Fix.** `excludePurchaseFromRevenue` now runs inside a transaction: it still always flags the purchase, and if the linked delivery order's status is still `'pending'`, it additionally cancels that order in the same transaction — the identical `UPDATE orders SET status = 'cancelled' WHERE id = $1 AND status = 'pending'` `cancelOrderAndRestock` already uses for an ordinary customer cancellation, scoped the same way so it's a no-op (and correctly leaves the order alone) for anything already accepted/picked-up/delivered. `PUT /api/super-admin/disputes/:id/resolve`'s 'void' branch now also broadcasts `order:updated` to the same rooms any other order cancellation already notifies, so an admin watching the dashboard live sees the order drop out of Pending Assignment immediately — not just the next time a report happens to be generated.

**Verified.** `node --check` passed on `server.js` and `db.js`. Hand-run against a real disposable local Postgres database seeded with two voided purchases — one linked to a still-`pending`, unaccepted order, one linked to an order already `accepted` by an agent: the pending one flipped to `cancelled` exactly as intended, while the already-accepted one was correctly left untouched, matching the existing "delivery company keeps their fee once work has started" rule.

**Follow-up — the fix above only stops this from happening again; it doesn't retroactively repair `ORD-MT2QZ97GM` itself (or any other order already stuck this way).** Deploying the code change alone leaves that specific order exactly as broken as before — the code path that would have cancelled it already ran, once, back when it was originally voided, before this fix existed. It also explained why the Delivery Company's own order list still showed it: that view reads the exact same `orders.status`, so of course it never disappeared from there either — it was never going to clear from *any* view until the underlying row itself changes.

Added a one-time cleanup directly to `schema.sql`: `UPDATE orders o SET status = 'cancelled' FROM purchases p WHERE p.delivery_order_id = o.id AND p.excluded_from_revenue = true AND o.status = 'pending'`, placed right after the `excluded_from_revenue` columns are declared. `schema.sql` runs in full on every `db.init()` — i.e. every server boot — so this sweeps up `ORD-MT2QZ97GM` and any other historically-stuck order the moment the app is next deployed/restarted, with no manual SQL required. It's naturally idempotent (a cancelled order no longer matches `o.status = 'pending'`, so re-running it on every future boot is always a no-op afterward), matching this file's existing precedent for this exact pattern (see the "Free Premium removed entirely" cleanup UPDATE on `vendor_subscriptions` above).

**Verified.** Re-ran the full `schema.sql` against a real disposable local Postgres database three times in sequence: applied fresh, seeded a purchase in exactly the pre-fix broken state (`excluded_from_revenue = true`, linked order still `pending` — reproducing `ORD-MT2QZ97GM`'s real state), re-applied `schema.sql` to simulate a redeploy — confirmed the order flipped to `cancelled` — then applied it a third time and confirmed no error and the row stayed `cancelled` (idempotent).

**This one still needs an actual deploy to take effect** — nothing in a delivered zip changes the live app until its code (now including this backfill) actually runs against the production database.

## Follow-up: after deploying, the order was STILL stuck — because it was never actually voided

**Reported:** after confirming the deploy above had gone out and the server had restarted, the order still showed in Pending Assignment on the Admin dashboard, with no delivery company having accepted it.

**Root cause — a second, different gap, not the same bug recurring.** `PUT /api/super-admin/disputes/:id/resolve` has three decisions: `void`, `refund`, `reject`. Only `void` was ever fixed (both times above). But resolving a dispute with `refund` — giving the customer their money back — is just as final from a delivery standpoint, and had **no path at all**, before or after either previous fix, to clearing a still-pending linked order: `refund` never sets `excluded_from_revenue` (a deliberately separate concept — whether a sale still counts toward revenue totals, unrelated to whether it's a lost cause for delivery), so it never touched `excludePurchaseFromRevenue`'s cleanup logic, and there is no other admin-facing "cancel this order" control anywhere in the app (`updateOrderStatus()` is only ever called with `'picked-up'` or `'delivered'` — never `'cancelled'`). If `ORD-MT2QZ97GM`'s dispute was resolved with a refund rather than a void, that fully explains why it survived both earlier fixes untouched.

**Fix.** Extracted the "cancel the linked order only if it's still `pending`" logic (previously inlined in `excludePurchaseFromRevenue`) into a shared transaction helper, `cancelPendingDeliveryOrderInTx`. Added `db.cancelPendingDeliveryOrderForPurchase(purchaseId)`, called from the `refund` branch the same way `void` already calls `excludePurchaseFromRevenue` — same live `order:updated` broadcast to the same rooms either way. `reject` deliberately still does nothing: a rejected dispute means the original order stands, unaffected — there's nothing to clean up.

Added a matching one-time `schema.sql` cleanup for this path too, right after the first one: any dispute `status = 'resolved'` with a `refund_amount` set (the only decision that ever sets one — `void` never does, `reject` sets `status = 'rejected'` instead) whose linked order is still `pending` gets cancelled, the moment the app is next deployed/restarted. Same idempotent, runs-on-every-boot pattern as the `void` cleanup right above it.

**Verified.** `node --check` passed on `server.js` and `db.js`. Hand-run against a real disposable local Postgres database seeded with three cases side by side: a refund-resolved dispute with its order still `pending` (flipped to `cancelled`, as intended — this is `ORD-MT2QZ97GM`'s actual scenario), a refund-resolved dispute whose order was already `accepted` (correctly left untouched), and a *rejected* dispute with its order still `pending` (correctly left untouched — rejecting a dispute isn't a reason to cancel anything). Re-ran `schema.sql` a third time afterward and confirmed all three rows held steady (idempotent).

**Same caveat as before: this needs an actual deploy to take effect.** If the dispute behind this specific order really was resolved with a refund (not a void), this new fix — once deployed — is what finally clears it; the two earlier fixes in this file were correct but didn't cover this path.
